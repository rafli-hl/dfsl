...
| 14 | ~~Structure C: abstract and Conclusion not yet revisited~~ | **Closed 2026-08-27.** The abstract's last two moves were rewritten, not merely checked: what predictability buys is now the transfer result, and the $\sqrt T$ drift factor is reframed from a caveat on a rate the paper offers into one of three measured obstructions making the rate *permanently* per-regime. The anchor line — *the contribution is the measurement and the stability, not an accuracy win* — is kept verbatim. §7 gained the transfer clause; its Limitations already carried the permanence language from D1B. One sentence was cut for the page budget and is recorded in `research_decisions.md` D-4 rather than dropped silently. Landed at 8.959pp. Original note follows. |
| 15 | Merge the research branches to `master` | **Open.** `master` is at `5e6c383` and is now 15+ commits behind: it contains none of D1C, T4, T4B, the grid gate or Structure C. The recoverable pre-restructure baseline is the `pre-structure-c` tag rather than a branch (`research_decisions.md` D-2). Three research branches are pushed and unmerged. |
| 14b | Structure C: original note | **Open at the time.** Step 1 changed §3's spine to `thm:transfer` and demoted the regret bound to Appendix D, and updated the third contribution to match. The abstract and §7 have not been checked against it. The register to keep is the abstract's existing one — the contribution is the measurement and the stability, not an accuracy win — so this is a consistency pass, not an upgrade. |
| 13 | Venue: is ICLR 2027 still the right target? | **Open decision for the author; raised 2026-08-27, and to be reassessed against the built document once Structure C is finished — a paper whose stated contribution is a mapped boundary plus a verified prediction is a different submission from one led by a regret theorem with caveats.** This tracker has carried "ICLR 2027" as settled since the venue moved, and the work has changed underneath it: four registered theory directions, all closed, three with *measured* obstructions, plus a taxonomy correction and a rigorous measurement study. That is good work poorly shaped for a main-track slot optimising for novelty, and well shaped for a venue where the measurement or the negative result is the contribution. Not a recommendation — a decision that should be made deliberately rather than by default. |
