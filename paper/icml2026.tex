...
\begin{remark}[Tail vs.\ scale drift; the drift is a genuine rate]
...
upward variation. On a persistently fluctuating scale $W_s$ is a sum of positive
increments that accumulates \emph{linearly} in $T$ ($W_s\sim T$; we verify this directly
in \cref{sec:tracker}), so $\sqrt{W_s}\sim\sqrt T$ is a \emph{genuine rate}: the honest
reading of \eqref{eq:mainbound} is $T^{1/p}$ \emph{per regime}, or $T^{1/p+1/2}$ over a
horizon with a growing number of regimes. What the tracker controls is the \emph{constant}
in $W_s$, not its exponent: updating on the regime timescale rather than every round
(\cref{rmk:tracker}) shrinks $W_s$ by orders of magnitude at a fixed horizon (\cref{sec:tracker}),
which keeps the bound useful in practice but does not make the drift asymptotically free.
Dividing by $s_t$ still avoids a $\sup_t\sigma_t$ \emph{multiplier}.
\end{remark}
