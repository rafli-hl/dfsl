...
across \emph{both} protocols---SN-OMD, with a predictable block-median scale, is the most
accurate per-row and normalized-GD per-timestep---so what singles out SN-OMD is a
\emph{predictable} scale, exactly what the dynamic-regret guarantee needs, not a uniform
accuracy win.
\end{abstract}
...
\begin{itemize}
  ...
  recurs, in weaker form, on image-model training gradients (\cref{app:mnist}). This is the
...
homogeneous, though: stable learning-rate ranges span two orders
...
distinctly buys is an interior accuracy optimum when the \emph{normalized} tail is genuinely
heavy (\cref{sec:msweep}); on Jane that tail is indistinguishable from causal-estimation cost,
so the optimum is flat and the cap-family members are close. SN-OMD's block-bootstrap standard
...
block-median scale removes it (per-row std $0.11\!\to\!0.04$;
...
one is best is tracker-dependent:} per-row the winner rotates window to window (fixed-$\tau$,
AdaGrad-Norm, and normalized-GD each lead some), and per-step normalized-GD is the most robust
($0.29\!\pm\!0.02$, best on $8/10$). With its \emph{deployed} winsorized-EMA tracker SN-OMD
stays bounded but is the most \emph{variable} member (per-row $0.14\!\pm\!0.11$, negative on
one window); swapping in the block-median tracker (\cref{sec:tracker}) removes that variance
and makes SN-OMD the \emph{best} bounded method per-row ($0.31\!\pm\!0.04$, never negative,
$0/10$ divergences). So the excess variance is the EMA tracker's, not the cap's---but the
...
member dominates across both protocols}: SN-OMD with the block-median tracker leads per-row
($0.31\!\pm\!.04$) and normalized-GD per-timestep ($0.29\!\pm\!.02$). SN-OMD is the only method
shown with two tracker variants; its block median was selected on window~1 as the most
accurate tracker, so this per-row lead is a best-case tracker choice, whereas every baseline
uses a single fixed configuration.}
...
Normalized-GD ($M\!\to\!0$)            & $0.18\pm.01$ & $0/10$ & $\mathbf{0.29}\pm.02$ & $0/10$ \\
Fixed-$\tau$ clip                      & $\mathbf{0.21}\pm.03$ & $0/10$ & $0.20\pm.13$ & $0/10$ \\
AdaGrad-Norm                           & $0.20\pm.07$ & $0/10$ & $0.19\pm.03$ & $0/10$ \\
\textbf{SN-OMD ($M{=}5$, ours)}        & $0.14\pm.11$ & $0/10$ & $0.13\pm.14$ & $0/10$ \\
\quad\emph{+ block-median tracker}     & $\mathbf{0.31}\pm.04$ & $0/10$ & $0.20\pm.13$ & $0/10$ \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
test} (\cref{app:sweep}): on a synthetic stream with an \emph{imposed} heavy normalized tail an
interior optimum appears (the best cap beats both endpoints), while on Jane---where the
normalized tail is indistinguishable from causal-estimation cost (\cref{tab:residual})---there
is \emph{no} meaningful interior optimum and the three members are comparable. So the cap buys
\emph{accuracy} only when the tail survives normalization, and \emph{stability unconditionally}.
...
on Jane is a block median; stress-tested against the circular block bootstrap its edge over
normalized-GD is real yet narrow and protocol-specific (\cref{app:tracker}), so the ``ties
normalized-GD'' headline stands where measured most conservatively. Past the tuned range both
trackers degrade without diverging ($O(\sqrt t)$ bound; \cref{app:tracker}).
...
\emph{predictable} tracked scale (not the current gradient) and in targeting \emph{dynamic}
regret under a \emph{drifting} scale rather than static convergence. Our normalization is
thus a streaming, scale-decoupled variant whose value is specifically stability under that
drift.
...
head-to-head on Jane (\cref{tab:jane}) it stays bounded and is accuracy-competitive (best
per-row), placing it with the bounded scale-free family. What it lacks is a
...
heavy-tailed (volatility clustering of a heavy-tailed conditional scale, not something the
bounded drift can produce) while the causally normalized gradient is much lighter. SN-OMD
normalizes and caps at a scale-free constant, so its iterates provably cannot diverge
($O(\sqrt t)$ for any rate) and attain $T^{1/p}$ \emph{per regime} (the drift is a genuine
$\sqrt{W_s}\!\sim\!\sqrt T$ factor). Across ten disjoint windows the \emph{bounded scale-free}
family stays bounded where OGD and scale-dependent clipping diverge, with no member dominating
across protocols (\cref{tab:replication}); what distinguishes SN-OMD is a \emph{predictable}
tracker---which the dynamic-regret guarantee requires---not an accuracy win.

\paragraph{Limitations.} SN-OMD is not fully tuning-free (setting-dependent learning rate;
untuned $M$, decay, winsorization; though its stable range far exceeds the baselines').
...
but not run head-to-head (AdaGrad-Norm and a constant clip are); and validation beyond our two
domains is warranted. Finally, on this stream the normalized residual tail is dominated by
\emph{causal-estimation cost}: a zero-residual GARCH surrogate reads $\hat\alpha\approx3.9$--$4.8$
after the same causal tracking and the real $3.73$ sits just below it (\cref{tab:residual}), so
we cannot separate a genuine residual tail from a scale less predictable than GARCH(1,1)---either
way $\hat\alpha\approx3.7$ leaves $p>2$, so both the $p\in(1,2]$ machinery and the cap's
tail-truncation are worst-case guarantees with no purchase here (the cap's role is the stability
of \cref{thm:stability}).
...
replicate across windows, where the robust leaders are normalized-GD (per-step) and SN-OMD
with a block-median scale (per-row)---see \cref{tab:replication}. Interpretation, and the
...
&= \tilde O\!\Bigl((\textstyle\sup_t\sigma_t)\,(D+\sqrt{D P_T})\,T^{1/p}
   \log(1/\delta)\Bigr),\nonumber
\end{align}
where $S_1=\sum_t\sigma_t$. The $\sqrt{\log(1/\delta)}$ multiplier and the additive
$\sup_t\sigma_t$ term are the Freedman variance and range contributions (\cref{lem:freedman};
the curvature concentration of step~(A) in \cref{app:proofs} is of the same order), so the
first line is a genuine high-probability bound. The second line is its \emph{bounded}-scale
simplification ($W_s=O(1)$, with $S_1\le T\sup_t\sigma_t$); \textbf{this assumption fails on
our data}, where $W_s\sim T$ (\cref{sec:tracker}), so the $T^{1/p}$ form is per-regime---see
the remark below.
\end{theorem}
...
\]
Under $M=\Theta(T^{1/p})$ and the tuned $\eta$ below, both deviation terms are of the same
$T^{1/p}$ order as the mean after the $\eta$-balance and are absorbed into the
$\sqrt{\log(1/\delta)}$ factor and additive range term of \eqref{eq:mainbound}. Their
\emph{constant}, though, is worth displaying: relative to the conditional mean
$M^{2-p}\kappa'S_1$, and writing $\bar\sigma:=S_1/T$ for the mean scale,
\[
...
range branch. On a spiking scale this ratio is far from $O(1)$; it is large precisely because
of the volatility clustering this paper studies (\cref{sec:experiments}), so the price of a
high-probability curvature bound is itself a form of the scale drift---a genuine constant cost,
not a benign one. This is the step that makes term~(A)---like (B)---a high-probability, not an
...
\paragraph{The cap sweep (\cref{fig:msweep}).} Sweeping $M$ at each cap's own tuned step turns
the cap into a \emph{mechanism test}: normalization already removes most of the tail
($\hat\alpha\!:2.4\to2.9$--$3.7$), so a finite cap should earn its keep only when the
\emph{normalized} gradient is still heavy. With an \emph{imposed} heavy normalized tail
($p=1.5$) the finite-$M$ interior optimum beats the uncapped endpoint $\approx240\times$
(heavy-tail instability) and the normalized-GD endpoint $\approx10\times$ (magnitude-blind
oscillation), both outside the seed-noise band; on Jane, where the normalized tail is
indistinguishable from causal-estimation cost (\cref{tab:residual}), the same sweep has no
interior optimum and the three cap-family members are comparable (\cref{tab:jane}). Both
predictions of the mechanism test are thus borne out. The cap therefore buys \emph{accuracy}
only when the tail survives normalization, and \emph{stability unconditionally}: uncapped
scale-adaptive OGD diverges at $\mathrm{lr}\ge2$ while SN-OMD does not.

\subsection{A second domain: image-model gradients}
\label{app:mnist}
On the per-step gradient norm of a small CNN on MNIST ($2{,}814$ steps) the pooled tail is
heavy ($\hat\alpha\approx2.6$) and normalizing lightens it, but this is \emph{schedule-driven}:
within a constant-learning-rate window the tail is already light ($\hat\alpha\approx6$--$9$),
so the heaviness is scale decay, not within-phase drift. The within-regime effect---confirmed
by the shuffle/iid-null controls and the feature--residual interaction---we establish only on
the market data.
...
holding hyperparameters frozen from window~1, the block-median tracker cuts SN-OMD's per-row
across-window $R^2$ standard deviation threefold ($0.11\!\to\!0.04$) and lifts its mean from
$0.14$ to $0.31$---above every other bounded method and never negative---whereas the deployed
...
The accuracy gaps are easy to over-read, so we report block-bootstrap intervals on the
aggregate gap, each method at its tuned rate (\cref{sec:tracker}). The
