...
\emph{linearly}: $W_s$ sums positive increments over the tracker's update events, and
plotting it against the update count gives a straight line with a slope independent of the
block length $B$ ($W_s\approx1.7\,(T/B)$, linear-fit $R^2>0.98$). So $\sqrt{W_s}\sim\sqrt T$
...
give $\beta<1$ at large $B$, but that is a finite-horizon artifact: at $B{=}10^4$ only
$\sim\!20$ blocks fit in our record; restricting the fit to horizons with $\gtrsim50$
blocks returns $\beta\approx1$.) What the update timescale \emph{does} control is the
...
$W_s,P_T^s\lesssim\sup_t\sigma_t\,(1,P_T)$.
\end{theorem}
...
regime}, or $T^{1/p+1/2}$ over a horizon with a growing number of regimes. The tracker's
...
median per block) on the $357\mathrm{k}$-round gradient-scale process. Restricting the
$W_s\sim T^{\beta}$ fit to horizons with $\gtrsim\!50$ blocks gives $\beta\approx1$
throughout (the small-$\beta$ a naive fit reports at large $B$ is a finite-horizon
artifact---too few blocks in the record); what $B$ controls is the constant, shrinking
$W_s$ by $\sim\!5$ orders of magnitude while roughly holding the lower bracket. Escaping
the resulting $\sqrt T$ drift factor needs a switching bound in the regime count, left
open.}
\label{tab:beta}
\centering
\begin{small}
