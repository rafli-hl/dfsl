...
\begin{remark}[Tail vs.\ scale drift; and when the drift \emph{is} a rate]
The tail index $p$ sets the exponent (through $M^{2-p}=T^{(2-p)/p}$, giving $T^{1/p}$),
while the scale drift enters through the factor $\sqrt{W_s}$. This is a benign
\emph{constant} only when the tracker's upward variation $W_s$ is bounded---i.e.\ when
the scale changes finitely often. It is \emph{not} free on a persistently fluctuating
scale: $W_s$ is a sum of positive increments and generally accumulates with $T$. On our
data we measure $W_s\sim T^{\beta}$ with $\beta\approx1$ (\cref{sec:tracker}), so the
honest reading of \eqref{eq:mainbound} is an effective rate $T^{1/p+\beta/2}$---the
bound is $T^{1/p}$ \emph{per regime}, and the drift term is a genuine cost, not a
constant, over a horizon with growing scale variation. Dividing by $s_t$ still avoids a
$\sup_t\sigma_t$ multiplier; what it cannot do is make an unboundedly wandering scale
free.
\end{remark}
...
\paragraph{Negative control (which mechanism needs drift?).} On a single quasi-stationary
$100\mathrm{k}$-row slice, plain OGD ($R^2\approx0.02$) beats the robust \emph{clippers}
($0.013$--$0.015$): their tail-rejection machinery does not pay without drift to survive.
Scale-invariance is a \emph{separate} axis, and it does still pay here---SN-OMD reaches
$\approx0.25$, because its gain is a larger stable step, not tail-rejection. So the
control cleanly separates the two mechanisms: clipping's robustness needs drift, whereas
scale normalization helps whenever the scale varies at all. Neither advantage is an
artifact of how the multi-regime stream is constructed.
...
drift to within a small factor ($W_s\approx6.7\,V_\sigma^+$). \textbf{The drift is not a
free constant, however.} Measuring how the upward variation grows with the horizon on
this process, both $V_\sigma^+$ and $W_s$ scale as $T^{\beta}$ with $\beta\approx1$---the
scale keeps fluctuating, so its accumulated upward variation grows roughly linearly.
Through the $\sqrt{W_s}$ term this makes the effective rate $T^{1/p+\beta/2}$ over a
horizon of growing scale variation: \cref{thm:regret}'s $T^{1/p}$ is a \emph{per-regime}
statement, and the scale drift is a genuine cost, not a constant. \cref{app:bracket}
lifts the lower-bracket half to a proposition (\cref{prop:tracker}) under a
piecewise-slowly-varying drift model; that model's bounded-regime idealization is exactly
where $W_s$ would be $O(1)$, and the measured $\beta\approx1$ is the gap to it.
...
rate $T^{1/p}$ per regime, with the scale drift entering as $\sqrt{W_s}$ (a genuine cost
once the scale variation grows). Empirically it stays bounded across the whole tested
