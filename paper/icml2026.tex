...
is strongly \emph{volatility-clustered}---predictable and persistent, drifting several-fold
across the record with overnight jumps---and this serial dependence, not a bounded drift or
a static scale mixture (neither of which can move a tail index), is what makes the
\emph{pooled} gradient look heavy-tailed (tail index $\hat\alpha\approx2.4$). The
\emph{causally scale-normalized} gradient---the quantity a learner should actually act
on---is markedly lighter ($\hat\alpha\approx2.9$--$3.7$, estimator-dependent), and an
order-\emph{shuffle} control that preserves the marginal removes the lightening entirely,
confirming it is the temporal dependence, not the estimator, that is exploited; the
difficulty is thus dominated by predictable nonstationarity rather than by an intrinsically
heavy tail. In this regime plain online
...
\begin{itemize}
  ...
  large market dataset the gradient scale is strongly \emph{volatility-clustered}
  (predictable serial dependence; Ljung--Box $Q_{100}\!\approx\!7\!\times\!10^5$, drifting
  $\sim\!6\times$ as its low-frequency component), and this clustering---not a bounded
  drift or a static scale mixture, which provably cannot move a tail index---makes the
  \emph{pooled} gradient tail heavy ($\hat\alpha\approx2.4$), while the
  scale-normalized gradient is much lighter ($\hat\alpha\approx2.9$--$3.7$). A
  marginal-preserving order-\emph{shuffle} abolishes the lightening (and a GARCH surrogate
  with light innovations reproduces it), and the effect recurs, in weaker form, on
...
\caption{\textbf{The scale is volatility-clustered; the pooled tail is a
predictable-conditional-scale effect.} (a) The
...
noisy. Normalizing by the causally-tracked scale lightens the tail to
$\hat\alpha\approx2.9$--$3.7$ (estimator-dependent); a marginal-preserving order-shuffle
removes this lightening entirely, so the pooled heaviness is predictable serial dependence,
not the marginal (\cref{sec:experiments}).}
\label{fig:problem}
\end{figure*}

\paragraph{The scale is volatility-clustered; the pooled tail is a predictable-conditional-scale effect.} At
...
pooled heaviness, however, is a signature of \emph{predictable} nonstationarity, not an
intrinsic tail. The gradient magnitude is strongly serially dependent---slowly-decaying
autocorrelation (lag-$100\approx0.26$) and Ljung--Box $Q_{100}\approx7\!\times\!10^{5}$
($p\approx0$; an iid null gives $\approx100$)---i.e.\ volatility clustering, of which the
$\approx6\times$ daily drift (\cref{fig:problem}b) is the low-frequency component. Dividing
by a causally-tracked robust scale $s_t$---the quantity SN-OMD actually acts on---lightens
the tail markedly, to $\hat\alpha\approx3.7$ (causal winsorized-EMA $s_t$), $\approx3.2$
(non-causal median), or $\approx2.9$ (trailing median), i.e.\ toward finite higher moments.
Three controls (released \texttt{research\_null\_normalization.py}) show this is the
temporal dependence---not the estimator, and not a static drift: (i) a marginal-preserving
order-\emph{shuffle} of the real series removes the lightening entirely ($\hat\alpha$ back
to $2.4$); (ii) the same causal normalization is tail-index \emph{preserving} on iid
streams of known index ($2.0,2.4,3.0$), so it cannot manufacture lightening; and (iii) a
GARCH surrogate with \emph{light} (Gaussian) innovations reproduces both the pooled
$\hat\alpha\approx2.4$ and its recovery to $\approx3.7$ under the tracker, whereas a bounded
drift or a static long-regime scale mixture provably cannot. So the operative difficulty is
a predictable, clustered scale, leaving only a mild residual heavy tail. (Per-day $\hat\alpha$ dips below $2$ on a minority of days, but
