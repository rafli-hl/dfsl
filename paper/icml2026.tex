...
leave it heavy), and a \emph{non-causal} tracker of the same surrogate reaches only $4.25$: the
causal-to-non-causal gap is just ${\approx}0.5$, so $3.77$ is a genuine zero-tail floor, not a
causal-estimation artifact. Yet the real normalized tail sits at $2.26$, \emph{well below} that
floor (and its own non-causal tracker lifts it by only $+0.14$, to $2.40$). Unlike Jane---whose
real $3.73$ sits \emph{at} its surrogate floor, i.e.\ no genuine tail beyond estimation cost
(\cref{tab:residual})---crypto's leftover is a \emph{genuine} heavy innovation tail (residual
$\hat\alpha\approx2.68$) that scale-tracking does not remove. (This ${\approx}0.5$ surrogate
estimation gap is far below Jane's ${\approx}4.5$; \cref{tab:residual} shows such gaps shrink with
the surrogate's volatility persistence, so the difference is calibration---our EMA path is smoother
than a GARCH(1,1)---not the tracker behaving differently.) This is the honest scope of the second
market: the drift-driven \emph{removability} replicates (the shuffle control), but crypto also
carries genuine innovation heaviness Jane lacks---exactly the regime where the cap earns its keep,
as the sharper stability dichotomy below confirms.
