...
tool. A light-innovation surrogate---crypto's own volatility path fed \emph{Gaussian} innovations,
so \emph{zero} genuine tail by construction---normalizes causally to $\hat\alpha\approx3.8$, so
causal estimation costs almost nothing here (the real oracle-vs-causal gap is only $+0.14$: $2.26$
to $2.40$). Yet the real normalized tail stays at $2.26$, \emph{well below} that zero-tail floor.
Unlike Jane---whose $3.73$ sits \emph{at} its surrogate floor, i.e.\ no genuine tail beyond
...
but crypto also carries genuine innovation heaviness Jane lacks---which is exactly the regime where
the cap earns its keep, as the sharper stability dichotomy below confirms.
