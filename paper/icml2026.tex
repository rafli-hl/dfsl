...
(\cref{fig:mechanism}). \emph{Scale-invariant} methods---normalized-GD, uncapped
scale-adaptive OGD, and SN-OMD---stay bounded to $\mathrm{lr}=2$ and reach $R^2\approx
0.2$--$0.3$. Per-timestep batching lifts OGD from $0.02$ to $0.14$ (averaging the
...
accuracy is not a contemporaneous-information artifact.
...
\paragraph{Negative control.} Robustness should pay only where there is drift to survive.
On a single quasi-stationary $100\mathrm{k}$-row slice (one short window, little regime
variation), plain OGD is in fact the \emph{best} method under causal standardization
($R^2\approx0.02$, vs.\ $0.013$--$0.015$ for the robust clippers); the ordering reverses
only on the multi-regime stream. This is the control the thesis predicts, and it guards
against reading our advantage as an artifact of the stream construction.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig3_main.png}
\caption{\textbf{Main result} (continuous multi-regime stream, \emph{causal}
standardization, per-row). (a) Weighted $R^2$ vs.\ learning rate: SN-OMD climbs
monotonically to $\approx0.28$ and never diverges, while OGD and both clippers cliff
above $\mathrm{lr}=10^{-2}$ (off-scale markers). (b) Peak rolling loss: SN-OMD stays
bounded ($\approx5$) everywhere; the others explode.}
\label{fig:main}
\end{figure*}
...
stay bounded to $\mathrm{lr}=2$ and are far more accurate. SN-OMD and normalized-GD are
\emph{comparable} here; the cap's role is non-divergence (uncapped diverges at
$\mathrm{lr}\ge2$). $R^2$ is not comparable to competition scores (different protocol,
see text).}
\label{tab:jane}
\centering
\begin{small}
