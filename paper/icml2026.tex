...
the best bounded baseline (a fixed clip plus normalization), both well clear of the rest, while
...
\emph{ties} the fixed-clip-plus-normalization baseline at the top ($+0.004$, CI
...
normalized-GD, whose window-1-optimal rate overfits to $0.09\!\pm\!.21$ per-step). SN-OMD is the
only method with two tracker variants (block selected on window~1).}
...
\cref{app:tracker})---so it belongs squarely in the bounded scale-free family. A fixed clip plus
full normalization is thus \emph{not} categorically worse than a tracked scale on this stream:
per-row the two tie and neither leads both protocols. What the \emph{predictable} tracked scale
adds is therefore the dynamic-regret guarantee, not a measured accuracy edge.
...
tracker---which the dynamic-regret guarantee requires---not an accuracy win: with the
block-median scale it co-leads per-row (tied with a normalize-and-clip baseline), and no member
leads both protocols.
...
domains is warranted. Finally, the normalized residual tail here is dominated by
\emph{causal-estimation cost}: a zero-residual GARCH surrogate reads $\hat\alpha\approx3.9$--$4.8$
and the real $3.73$ sits just below it (\cref{tab:residual}), so with $p>2$ the $p\in(1,2]$
tail machinery has little purchase here---the cap is for stability (\cref{thm:stability}).
...
it ($+0.11$, $[+0.05,+0.17]$, $8/10$), as it does the likewise-collapsed CM. Where block ties
...
The one axis we do \emph{not} tune is SN-OMD's cap $M$, fixed at $5$ a priori (Limitations).
That choice is conservative, not favorable: sweeping $M$ at the tuned rate, the block tracker's
per-row window-1 $R^2$ keeps \emph{rising} past $M{=}5$ (to $0.42$ at $M{=}10$), so the reported
number understates what a tuned cap would give.
...
\texttt{research\_cm\_widen.py}) and its momentum $\beta{=}0$ is the natural no-momentum floor;
its learning rate is interior on both protocols.}
\end{table}
