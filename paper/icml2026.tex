...
\paragraph{A budget heuristic (deferred to \cref{sec:lower}).} A per-regime budget argument
(\cref{sec:lower}) shows the achievable rate degrades smoothly with the tail ($T^{1/p}$
under a finite $p$-th moment) and that under drift the per-regime costs \emph{add}, so a few
short heavy-tailed ($p_r<2$) regimes dominate. We use this as motivation only: those
$p_r<2$ regimes are a pooled-tail artifact that largely evaporates once the gradient is
normalized (\cref{sec:experiments}), leaving the per-regime worst-tail rate $T^{1/p}$ that
SN-OMD attains rather than diverges.
...
\begin{table}[t]
\caption{\textbf{The residual index is a real, order-one property, not a tracker-lag
artifact.} Hill $\hat\alpha$ of the normalized $\norm{\gt}$ at three tail fractions $k$
(larger $=$ lighter). Every tracked scale lightens the raw pooled tail (the deployed
winsorized-EMA by $+1.0$ to $+1.5$); running the \emph{same} estimator non-causally
(two-sided, sees the future) lightens the residual only $\sim\!+0.3$ more. On a GARCH surrogate
calibrated to the real raw index---with Gaussian innovations, hence \emph{zero} residual tail
by construction---the \emph{causal} tracker still reads $3.9$--$4.5$ against a true-$\sigma_t$
oracle floor of $\hat\alpha\!\approx\!9$. The real $3.73$ sits just below this, so it is
consistent with pure causal-estimation cost: no genuine residual heavy tail is resolvable at
this precision.}
\label{tab:residual}
\centering
\begin{small}
...
positive at every threshold and estimator (\cref{tab:residual}), and what remains is
\emph{causal-estimation cost}, not a residual property of the data: isolating causality (the
\emph{same} estimator run non-causally) recovers only $\sim\!+0.3$ (two estimator families
agree, $+0.32$ EMA and $+0.31$ median), and on a GARCH surrogate with Gaussian
innovations---\emph{zero} residual tail by construction---the same causal tracker still
reports $\hat\alpha\approx3.9$--$4.5$ against a true-$\sigma_t$ oracle of $\approx9$. The real
$3.73$ sits just below that, so no genuine residual heavy tail is resolvable at this
precision: the normalized tail appears \emph{entirely conditional}, and what is left is the
price of not seeing the future. What makes it removable is \emph{serial dependence}: the gradient magnitude is strongly clustered---rank (Spearman)
...
more accurate, with \emph{none} dominating: the best per-protocol entry is the fixed-$\tau$
clip (per-step) or AdaGrad-Norm (per-row), not SN-OMD. Interpretation, and the block-median
tracker variant, are discussed in \cref{sec:experiments,sec:tracker}. $R^2$ is not
...
($\clip(\gt/s_t,M)=M\,\gt/\norm{\gt}$), so $M\to0$ is \emph{normalized-GD} and
$M\to\infty$ is uncapped \emph{scale-adaptive OGD}. Every \emph{finite}-$M$ member satisfies
\cref{thm:stability} ($\norm{\hat g_t}\le M$), the normalized-GD endpoint included; the
uncapped $M\!\to\!\infty$ endpoint is the lone exception ($\norm{\hat g_t}$ unbounded) and is
exactly the member that diverges (\cref{tab:jane}). So against the other stable member,
normalized-GD, non-divergence is not what singles out SN-OMD; the point is that a finite $M$
\emph{preserves gradient magnitude within the bulk} and truncates only the tail, whereas
normalized-GD discards magnitude every round.
...
\emph{normalized} gradient is still heavy. On a synthetic stream with an \emph{imposed}
heavy normalized tail ($p=1.5$; \cref{fig:msweep}) an interior optimum appears---the best
cap beats the uncapped end $\approx240\times$ (heavy-tail instability) and the normalized-GD
end $\approx10\times$ (magnitude-blind oscillation), both outside the seed-noise
band---whereas on Jane, where the normalized tail is only mildly heavy, there is \emph{no}
meaningful interior optimum: the three cap-family members are comparable (\cref{tab:jane}).
Same mechanism, both predictions borne out. So the cap buys \emph{accuracy} when the tail
survives normalization, and \emph{stability unconditionally}: uncapped scale-adaptive OGD
diverges at $\mathrm{lr}\ge2$ while SN-OMD does not.

\paragraph{Synthetic streams with a known tail (\cref{sec:synthetic}).} On synthetic
streams with a controlled Student-$t$ index $p$, plain OGD's fitted cumulative-regret
exponent rises to $1.55$ (super-linear, divergent) as $p$ falls to $1.3$, reproducing the
real-data divergence with a \emph{known} tail index, so the tail is the mechanism, not a
learning-rate artifact; SN-OMD stays sublinear. Full exponents, and the dynamic-regret
path-length fit, are in the appendix.
...
requirement is one-sided (\cref{sec:method})---hold the lower bracket
$s_t\ge\tfrac12\sigma_t$ everywhere while keeping the upward variation $W_s$ (entering
\cref{thm:regret} as $\sqrt{W_s}$) near the true drift $V_\sigma^+$. A single-timescale
...
on Jane is a block median, but stress-tested against the circular block bootstrap
(\cref{app:tracker}) its edge over normalized-GD is real yet narrow ($0.38$ vs.\ $0.197$
per-row) and evaporates under the batched protocol (interval straddling zero), so the ``ties
normalized-GD'' headline (\cref{tab:jane}) stands where measured most conservatively---using
the block bootstrap rather than a paired-fold $t$-test, since the aggregate $R^2$ is a ratio
of sums whose fold-mean is not the aggregate. Past the tuned range both trackers degrade
without diverging ($O(\sqrt t)$ bound; \cref{app:tracker}).
...
under heavy-tailed noise \citep{zhang2020clip,gorbunov2020,nguyen2023}. Our normalization is
a streaming, scale-decoupled variant whose value is specifically stability under a
\emph{drifting} scale.
...
goes back to \citet{zinkevich2003}; scale-free methods adapt to unknown gradient scale
\citep{orabona2018}; strongly-adaptive and parameter-free wrappers adapt to unknown
...
heavy-tailed---a volatility-clustering effect of a heavy-tailed conditional scale
\citep{kesten1973}, not something the bounded drift can produce \citep{breiman1965}---while
the causally normalized gradient is much lighter. SN-OMD decouples the update from this tail
scale by normalizing and capping at a scale-free constant, so its iterates provably cannot
diverge ($O(\sqrt t)$ for any learning rate) and attain the worst-tail rate $T^{1/p}$
\emph{per regime}; the drift is a genuine rate ($\sqrt{W_s}\!\sim\!\sqrt T$), so a
regime-timescale tracker shrinks the constant but not the exponent (\cref{sec:tracker}). It
is one member of a broader \emph{bounded scale-free} family that stays bounded where OGD and
scale-dependent clipping diverge; on this stream no member dominates and SN-OMD ties
normalized-GD (\cref{tab:jane}). What distinguishes it is not accuracy but a
\emph{predictable} tracker---which the dynamic-regret guarantee requires---and a cap that
earns an accuracy optimum only when the normalized tail stays heavy (\cref{sec:msweep}).
...
covers only the two-timescale envelope (\cref{ass:track}), not the deployed EMA or the
per-row-best block median, whose regret under drift is supported empirically
(\cref{sec:tracker}); RMSProp/Adam are argued to diverge but not run head-to-head (though
AdaGrad-Norm and a constant clip are; \cref{sec:related,tab:jane}); and validation beyond
our two domains is warranted. Finally, on this stream the normalized tail appears
\emph{entirely conditional}---the residual $\hat\alpha\approx3.7$ is consistent with pure
causal-estimation cost, with no genuine heavy tail resolvable (\cref{tab:residual})---so both
the $p\in(1,2]$ machinery \emph{and} the cap's tail-truncation benefit are worst-case
guarantees with no purchase here: on this data the cap earns no accuracy advantage (its role
is the unconditional stability of \cref{thm:stability}), and the $(1,2]$ apparatus does not
bind.

\section*{Software and Data}
The released code provides the \texttt{ScaleNormalizedOGD} learner, the scale trackers
(winsorized EMA, two-timescale envelope, block median), the configs, and the scripts that
reproduce every figure and number. Data: the public Jane Street Real-Time Market Data
Forecasting competition, plus MNIST-CNN gradient norms (\cref{sec:experiments}).
...
of $\norm{\gt}$ is a constant multiple of $\sigma_t$ (regular variation). Under the
