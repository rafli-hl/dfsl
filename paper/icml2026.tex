...
\citet{cutkoskymehta2021}---stays bounded ($0/10$) but is the \emph{most variable} member
per-row and collapses per-timestep ($0.01$): a fixed clip and a fully normalized step do not
track the drift.}
...
Cutkosky--Mehta (2021)                 & $0.16\pm.16$ & $0/10$ & $0.01\pm.01$ & $0/10$ \\
...
head-to-head across the ten frozen windows (\cref{tab:replication}), their method stays
bounded ($0/10$ divergences, so it too is scale-free) but does \emph{not} match the
tracked-scale methods: it is the most variable member per-row ($0.16\!\pm\!.16$) and collapses
per-timestep ($0.01$). This is direct evidence that the \emph{predictable tracked scale}, not
clipping-plus-normalization per se, is what buys accuracy under drift.
...
fixed-$\tau$ clip by $+0.10$, and AdaGrad-Norm by $+0.11$. This is a \emph{measured} accuracy
...
is exactly the variable the effect lives in. The edge is \emph{protocol-specific}, though:
