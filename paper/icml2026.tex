...
earns an accuracy optimum only where the normalized tail stays genuinely heavy, which our
real stream does not reach.
\end{abstract}
...
\emph{predictable} part of a nonstationary scale; the residual is consistent with
causal-estimation cost, not a genuine heavy tail (\cref{tab:residual}). The
...
(\emph{zero} residual tail) and is calibrated to the real raw index, yet causal tracking
alone reads $\hat\alpha\approx4$--$5$ (oracle $\approx8$--$10$); the real $3.73$ sits at or
below this, consistent with pure causal-estimation cost---no genuine residual tail is
resolvable, and none is reducible by removing causality or (the surrogate shows) by better
causal estimation.}
\label{tab:residual}
\centering
\begin{small}
...
tracker still reads $\hat\alpha\approx4$--$4.5$ against a true-$\sigma_t$ oracle of $\approx9$
(\cref{tab:residual}). The real
...
(\cref{sec:msweep}); on Jane that tail is not distinguishable from causal-estimation cost
($\hat\alpha\approx3.7$; \cref{tab:residual}), so the optimum is flat and SN-OMD,
normalized-GD, and the uncapped end are close. One
...
band---whereas on Jane, where the normalized tail is not distinguishable from
causal-estimation cost (\cref{tab:residual}), there is \emph{no} meaningful interior
optimum: the three cap-family members are comparable (\cref{tab:jane}).
