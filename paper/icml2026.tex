...
\icmltitlerunning{Distribution-Free Sequential Learning under Heavy-Tailed Uncertainty}

\begin{document}

\twocolumn[
  \icmltitle{Distribution-Free Sequential Learning under\\ Heavy-Tailed Uncertainty}
  ...
  \icmlkeywords{online learning, heavy tails, distribution-free, nonstationary}
  \vskip 0.3in
]
...
\begin{abstract}
We study online prediction on streams whose loss gradients are heavy-tailed and,
crucially, \emph{nonstationary}: on a large financial dataset we document that the
gradient tail scale drifts several-fold and that the tail index itself
intermittently drops below two, into the infinite-variance regime. In this setting
plain online gradient descent (OGD) has no usable learning rate---small rates
underfit while any rate large enough to learn diverges when the scale spikes---and we
show that the standard remedy, clipping to a \emph{scale-dependent} threshold, is
fragile because the resulting step still inherits the scale drift. We propose
\emph{Scale-Normalized OMD} (SN-OMD), which divides the gradient by a tracked robust
scale and caps it at a \emph{scale-free} constant, making the step scale-invariant;
its iterates provably remain bounded for any learning rate or realization---a
stability guarantee distinct from convergence. We prove a
high-probability dynamic-regret bound of order $T^{1/p}$ under only a finite $p$-th
moment ($p\in(1,2]$), and show a matching lower bound implying that near-$\sqrt{T}$
\emph{dynamic} regret is information-theoretically unreachable once the tail is heavy
and nonstationary---a wall we measure directly on data. Empirically, SN-OMD never
diverges across a $1000\times$ learning-rate range and is the most accurate method,
where OGD and clipping both fail.
\end{abstract}
...
heavy tail is \emph{nonstationary}. Measuring the gradient of an online linear
predictor on the Jane Street market dataset, we find (i) the intrinsic gradient tail
index is $\hat\alpha\approx 2.4$---finite variance but infinite fourth moment---and is
an \emph{interaction} effect, heavier than either the features or the residuals alone;
(ii) the gradient \emph{scale} drifts roughly $6\times$ across two months with
overnight jumps; and (iii) the tail \emph{index} itself drifts, dipping below two
(infinite variance) on a nontrivial fraction of days. This combination---heavy tails
that are also nonstationary---is exactly where existing distribution-free online
learning results, which are either in expectation, on bounded domains, or against a
static comparator \citep{zhang2026oco,zhangcutkosky2022}, do not reach.
...
\begin{itemize}
  \item \textbf{Problem and lower bound (\cref{sec:problem}).} We formalize
  distribution-free online learning under a \emph{drifting} $p$-th moment and show
  that the achievable rate is $T^{1/p}$; on real data we measure that near-$\sqrt{T}$
  \emph{dynamic} regret is out of reach (the heavy-tail ``budget'' exceeds $\sqrt{T}$),
  while static regret is not---so the wall sits exactly at heavy-tails
  $\times$ nonstationarity.
  ...
  \item \textbf{Experiments (\cref{sec:experiments}).} On Jane Street, SN-OMD never
  diverges across a $1000\times$ learning-rate range and attains the highest weighted
  $R^2$, whereas OGD and scale-dependent clipping both diverge. We also empirically
  discharge the scale-tracking assumption---a two-timescale tracker holds the
  stability-critical bracket at every round---and release a tested implementation.
\end{itemize}
...
means the learner knows none of $\sigma_t$, $p_t$, $D$, or the comparator norm.
...
short but heavy regimes ($p_r<2$) dominate the sum. This additivity is what
distinguishes our limit from existing heavy-tailed lower bounds. Prior results fix a
\emph{single} tail index $p$, a stationary scale, and a \emph{static} comparator, and
recover the $T^{1/p}$ rate \eqref{eq:tp} \citep{zhang2026oco,zhangcutkosky2022,bubeck2013};
the obstruction we identify is not that per-regime rate but its \emph{additivity} under
drift. With a per-regime index $p_r$ and scale $\sigma_r$, dynamic regret \emph{sums}
the per-regime costs \eqref{eq:budgets}, and---since $L\mapsto L^{1/p}$ is convex and
$p_r$ may dip below $2$---a few short infinite-variance regimes push that sum
super-$\sqrt T$ even when every regime's own cost is benign. It is the interaction of a
\emph{drifting} exponent with a \emph{moving} comparator, absent from the static
single-$p$ analyses, that raises the wall; the static budget (a $\max$, not a sum) does
not see it. As \eqref{eq:budgets} is a
\emph{lower} bound, whenever its right-hand ``dynamic budget'' exceeds $\sqrt T$, no
algorithm can achieve near-$\sqrt T$ dynamic regret. We measure exactly this on Jane
Street (\cref{sec:experiments}): the dynamic budget is $\approx6.8\sqrt T$ at both a
$60$-day and the full $1{,}699$-day scale, whereas the static budget is only
$\approx0.16\sqrt T$. \textbf{Near-$\sqrt T$ \emph{dynamic} regret is thus
information-theoretically out of reach on this data}: the wall sits precisely at the
intersection of heavy tails and nonstationarity, and the best any method can do is
attain the worst-tail rate $T^{1/p}$ rather than diverge---which is what SN-OMD does.
...
\label{eq:mainbound}
R_T(u_{1:T})
&= O\!\Bigl(D\,S_1^{1/p}W_s^{1-1/p}+\sqrt{D\,P_T^s}\,S_1^{1/p}W_s^{-1/p}\Bigr)\\
&= \tilde O\!\Bigl((\textstyle\sup_t\sigma_t)\,(D+\sqrt{D P_T})\,T^{1/p}
   \log(1/\delta)\Bigr),\nonumber
\end{align}
where $S_1=\sum_t\sigma_t$. Together with \cref{thm:stability}, the iterate is bounded
...
The bound $D\,S_1^{1/p}W_s^{1-1/p}$ \emph{separates} the two effects: the tail index
$p$ sets the \emph{rate} exponent $T^{1/p}$, while the scale drift enters only as the
\emph{constant} $(W_s/\sup_t\sigma_t)^{1-1/p}$ (measured $\approx1.5\times$ on data).
Crucially, dividing by $s_t$ does not smuggle a $\sup_t\sigma_t$ \emph{rate} penalty.
\end{remark}
...
zero-mean $R^2$. We stream chronologically, standardize features online, and compare
plain OGD, quantile-based AdaptiveClip, RobustOMD (a scale-dependent robust-mean clip),
and SN-OMD. All measurements are learner-independent where possible (gradients at the
fixed best linear predictor $w^\star$), and every number is reproducible from the
released code.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig1_problem.png}
\caption{\textbf{The problem is real and nonstationary.} (a) The gradient tail
(Hill index $\hat\alpha\approx2.43$) is heavier than either its residual or feature
factors ($\hat\alpha\approx4$): an interaction effect. (b) The daily gradient tail
index across the full $1{,}699$-day record drifts and dips below $2$ (infinite
variance) on $\approx6\%$ of days.}
\label{fig:problem}
\end{figure*}

\paragraph{Heavy tails are real, and nonstationary.} At the fixed optimum $w^\star$,
the gradient-norm tail index is $\hat\alpha\approx2.43$ (finite variance, infinite
fourth moment) and is an \emph{interaction} effect: the residual and feature norms each
have $\hat\alpha\approx4$, but their product---the gradient---has a heavier tail
because extreme features and residuals co-occur (\cref{fig:problem}a). The heavy tail
is also doubly nonstationary: the daily gradient scale drifts $\approx6\times$ with
overnight jumps, and the tail index itself drifts, dipping below $2$ on $\approx6\%$
of days (\cref{fig:problem}b). Quantifying the lower bound of \cref{sec:lower}, the
\emph{dynamic} heavy budget $\sum_r\sigma_r L_r^{1/p_r}$ is $\approx6.8\sqrt T$ at both
$60$-day and full-record scales, while the \emph{static} budget is $\approx0.16\sqrt T$
--- confirming that near-$\sqrt T$ dynamic regret is unreachable here.
...
\paragraph{OGD has no usable rate; clipping is fragile.} It is tempting to report that
plain OGD ``diverges'' on this stream---at a shared learning rate it reaches weighted
$R^2=-286$---but this is a \emph{learning-rate artifact}: at that rate OGD is unstable,
yet at any smaller rate it underfits. Tuned to its own best rate on a single regime,
OGD reaches only $R^2\approx0.03$, forced to a tiny, safe step; on the continuous
multi-regime stream its best rate is in fact \emph{negative} ($R^2=-0.065$,
\cref{tab:jane}), because any rate large enough to compete diverges the first time the
gradient scale spikes. Scale-dependent clipping widens the stable range but remains
fragile, because its threshold $\tau_t=c\,s_t$ rescales \emph{with} the regime:
tracking the scale transmits the $6\times$ swing into the step, while smoothing it lags
regime onsets (\cref{fig:mechanism}). No single fixed rate is both aggressive enough to
learn on calm regimes and safe enough to survive turbulent ones.

\paragraph{SN-OMD attains the wall.} On a continuous multi-regime stream, SN-OMD stays
bounded across the \emph{entire} learning-rate range (peak rolling loss $\approx5$
throughout), while OGD diverges to $>10^{100}$ and both clippers cliff
(\cref{fig:main}). At its own learning rate SN-OMD attains the \emph{highest} weighted
$R^2$ (\cref{tab:jane})---scale-invariance helps both stability and accuracy, because
the step is well-conditioned in every regime. On per-window restarts (where clipping
diverged) SN-OMD is positive on every regime, including the turbulent one, and never
diverges; even mis-tuned it degrades gracefully rather than catastrophically.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig3_main.png}
\caption{\textbf{Main result} (continuous multi-regime stream). (a) Weighted $R^2$
vs.\ learning rate: SN-OMD climbs to $\approx0.24$ across the whole range, while OGD
diverges (off-scale markers) and the clippers cliff. (b) Peak rolling loss: SN-OMD is
bounded everywhere; the others explode.}
\label{fig:main}
\end{figure*}

\begin{table}[t]
\caption{Best weighted $R^2$ per method (each at its own tuned learning rate) on the
continuous Jane Street stream, and the stable learning-rate range. SN-OMD is both the
most accurate and the only method that never diverges.}
\label{tab:jane}
\centering
\begin{small}
\begin{tabular}{lcc}
\toprule
Method & Best weighted $R^2$ & Stable lr range \\
\midrule
OGD                 & $-0.065$ & $\le 2\!\times\!10^{-3}$ \\
AdaptiveClip        & $0.144$  & up to $3\!\times\!10^{-2}$ \\
RobustOMD (MoM)     & $0.155$  & up to $3\!\times\!10^{-2}$ \\
\textbf{SN-OMD (ours)} & $\mathbf{0.242}$ & $\mathbf{2\!\times\!10^{-3}\text{--}2,\ \text{no divergence}}$ \\
\bottomrule
\end{tabular}
\end{small}
\end{table}

\subsection{Which robust primitive? Fair per-method tuning}
\label{sec:fair}

SN-OMD normalizes by a robust scale; which robust-\emph{mean} primitive one clips or
averages with is a separate axis, and it is easy to draw the wrong conclusion about it.
Tuning each baseline to its own best learning rate on a fixed $100{,}000$-row slice
(\cref{tab:fair}), the \emph{hard-rejection} estimators---median-of-means and the
trimmed mean, which discard whole blocks or extreme order statistics---together with
quantile-based AdaptiveClip all beat Catoni's \emph{soft} log-truncation. Catoni's
influence function $\psi(t)=\operatorname{sign}(t)\log(1+|t|+t^2/2)$ is unbounded, so it
never \emph{fully} rejects an extreme gradient: it peaks at $R^2=0.079$ and destabilizes
at $\mathrm{lr}=3\!\times\!10^{-2}$, exactly where the hard-rejection methods are near
their best ($0.13$--$0.15$). This \emph{reverses} the ranking a single \emph{shared}
learning rate would report---at a conservative shared rate every method sits in its
indistinguishable linear regime and Catoni happens to look best---and is why we sweep
the learning rate per method throughout rather than comparing at one rate. The takeaway
for the practitioner: when the goal is to push the step size, which is where robustness
pays, prefer hard rejection to soft truncation.

\begin{table}[t]
\caption{Fair per-method tuning on a fixed $100{,}000$-row slice: best weighted $R^2$
at each method's \emph{own} tuned learning rate. Hard-rejection robust means and
quantile clipping beat the soft-truncation Catoni M-estimator---the reverse of what a
single shared learning rate reports.}
\label{tab:fair}
\centering
\begin{small}
\begin{tabular}{lcc}
\toprule
Method & Best weighted $R^2$ & at lr \\
\midrule
OGD                          & $0.028$ & $2\!\times\!10^{-3}$ \\
RobustOMD (Catoni)           & $0.079$ & $2\!\times\!10^{-2}$ \\
RobustOMD (trimmed mean)     & $0.128$ & $3\!\times\!10^{-2}$ \\
AdaptiveClip (quantile)      & $0.138$ & $3\!\times\!10^{-2}$ \\
RobustOMD (median-of-means)  & $\mathbf{0.149}$ & $3\!\times\!10^{-2}$ \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
enters the final bound only through the measured constant
$(W_s/\sup_t\sigma_t)^{1-1/p}\approx1.5\times$. \cref{app:bracket} lifts this from an
...
Heavy tails on real streams are also nonstationary, and this combination---not heavy
tails alone---is what makes distribution-free sequential learning hard: near-$\sqrt T$
\emph{dynamic} regret becomes information-theoretically unreachable. The right response
is to decouple the update from the tail scale: SN-OMD normalizes the gradient by a
tracked scale and caps it at a scale-free constant, which provably never diverges and
attains the worst-tail rate $T^{1/p}$, with the scale drift entering only as a
constant. Empirically it dominates OGD and scale-dependent clipping, which both fail.
...
gives the bound. (Verified numerically on $2000$ random instances.)
\end{proof}
...
\begin{equation*}
R_T(u_{1:T})=O\bigl(D\,S_1^{1/p}W_s^{1-1/p}+\sqrt{D\,P_T^s}\,S_1^{1/p}W_s^{-1/p}\bigr)
=\tilde O\bigl((\textstyle\sup_t\sigma_t)(D+\sqrt{D P_T})\,T^{1/p}\log(1/\delta)\bigr),
\end{equation*}
using $S_1\le T\sup_t\sigma_t$ and $W_s,P_T^s\lesssim\sup_t\sigma_t\cdot(1,P_T)$. The
stationary case $\sigma_t\equiv\sigma$, $p=2$ recovers $\sigma(D+\sqrt{DP_T})\sqrt T$,
the classical dynamic-OGD rate. \qed
