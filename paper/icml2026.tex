...
\begin{itemize}
  ...
  block-median scale the best bounded method per-row across ten frozen windows, though no
  single member dominates both protocols (\cref{tab:replication})---where OGD and
...
(the variant with a proven $W_s$ bound, analyzed below), and a block median (the most accurate
...
What singles out SN-OMD is therefore two things, not uniform accuracy, and we state them as
such. \emph{(i) Predictability, and a measured per-row edge:} its scale $s_t$ is a
...
magnitude every round---and with the block-median scale that predictability also buys a
\emph{paired-significant} per-row accuracy edge ($+0.13$ over normalized-GD, $+0.17$ over the
EMA, $10/10$ windows; \cref{app:tracker}), though not on both protocols. \emph{(ii) The cap:} a
...
uses a single fixed configuration. The closest prior---the normalize-and-clip method of
\citet{cutkoskymehta2021}, tuned over its momentum coefficient---stays bounded ($0/10$) and is
\emph{competitive} per-row ($0.29\!\pm\!.04$, second to SN-OMD$+$block, which still beats it by
a paired $+0.02$; \cref{app:tracker}); its per-timestep window-1 optimum is regime-fragile
($0.09\!\pm\!.21$).}
...
on Jane is a block median; across the ten frozen windows it is the best bounded method per-row
($0.31\!\pm\!.04$ vs.\ normalized-GD's $0.18\!\pm\!.01$), while per-timestep normalized-GD
leads---the accuracy verdict is protocol-dependent (\cref{tab:replication,app:tracker}). Past
...
coefficient), their method stays bounded ($0/10$) and is \emph{competitive}---per-row
$0.29\!\pm\!.04$, second only to the block-median SN-OMD, which still beats it by a paired
$+0.02$ ($10/10$; \cref{app:tracker})---so it belongs squarely in the bounded scale-free
family. A fixed clip plus full normalization is thus \emph{not} categorically worse than a
tracked scale on this stream; what the \emph{predictable} tracked scale adds is the
dynamic-regret guarantee and the (paired-significant, per-row) block-median edge, not a
wholesale accuracy gap.
...
head-to-head on Jane it stays bounded and is accuracy-competitive (best per-row on the tuned
window, $0.20\!\pm\!.07$ across ten; \cref{tab:jane,tab:replication}), placing it with the
...
across both protocols (\cref{tab:replication}); what distinguishes SN-OMD is a \emph{predictable}
tracker---which the dynamic-regret guarantee requires---and, with the block-median scale, a
paired-significant per-row accuracy edge, not a \emph{uniform} accuracy win.
...
(\texttt{research\_tracker\_bootstrap.py}). Per-row this is decisive: the block tracker beats
\emph{every} method with a paired $95\%$ CI clear of zero and $10/10$ window wins---the EMA
default by $+0.17$ ($[+0.11,+0.24]$), normalized-GD by $+0.13$ ($[+0.10,+0.15]$), the
fixed-$\tau$ clip by $+0.10$, AdaGrad-Norm by $+0.11$, and even the (retuned, competitive)
Cutkosky--Mehta by $+0.02$ ($[+0.015,+0.029]$). This is a \emph{measured} accuracy
result, and it \emph{supersedes} the single-window block-bootstrap gap of $+0.10$ (CI
$[-0.04,+0.23]$) we reported in earlier drafts: that instrument \emph{fixes} the regime, which
is exactly the variable the effect lives in. Nor is it a tracker-\emph{selection} artifact:
dropping window~1---on which the block median was chosen---leaves the per-row edge intact on
the nine untouched windows ($+0.18$ over the EMA, $[+0.11,+0.25]$, $9/9$; $+0.12$ over
normalized-GD, $[+0.10,+0.15]$, $9/9$). The edge is \emph{protocol-specific}, though:
per-timestep the block tracker does \emph{not} lead---it is statistically tied with the EMA
default (paired $+0.07$, $[-0.00,+0.15]$), the fixed-$\tau$ clip, and AdaGrad-Norm, while
normalized-GD \emph{beats} it ($-0.09$, $[-0.16,-0.01]$). The mechanism is substitution:
per-timestep batching averages the $\sim\!9$ correlated symbols per $(\text{date},\text{time})$
group, and that averaging is itself the variance reduction the smooth block scale supplies
per-row, so the two do not compound---when the protocol smooths the update, a smoother tracker
adds nothing (the same averaging lifts normalized-GD, $0.197\to0.316$).

\paragraph{Which tracker we report, and why.} We therefore report the block median as the
per-row \emph{accuracy-optimal} tracker---it is the significant best across regimes---while
...
$W_s$ guarantee is, the winsorized EMA as a cheap reactive default between them. We headline
the block median because ten frozen windows say it is the most accurate bounded method per-row,
not because it is the most analyzable---deploying the corner with no $W_s$ bound is a deliberate
choice the paired evidence forces, and we state it as one. Past the tuned range every tracker
