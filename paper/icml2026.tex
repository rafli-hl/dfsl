...
beats full normalization by ${+}0.11$ per-row, though the mid-range caps sit within noise. SN-OMD's per-row block-bootstrap standard
error ($\pm.079$) runs about twice normalized-GD's ($\pm.037$; \cref{tab:jane}, and it is no
wider per-step), which the ten-window replication traces to the winsorized-EMA tracker rather
than the cap: the block-median scale roughly halves it (per-row std $0.11\!\to\!0.07$;
