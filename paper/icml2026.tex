...
bracket for the smaller constant---but this is benign: running the $B{=}10^4$ tracker
through the whole learning-rate sweep, SN-OMD stays bounded (peak rolling loss $\le5.3$
throughout) and is if anything \emph{more} accurate and more stable than the
winsorized-EMA default.
...
\paragraph{Which tracker to deploy.} Both are bounded and usable; they trade reactivity
against upward variation, and we do \emph{not} claim one is uniformly more accurate. On
our stream a slow regime-timescale (block-median) tracker is in fact \emph{more} accurate
and more stable than the winsorized EMA at large learning rates (\cref{sec:tracker}), and
it also minimizes the $W_s$ constant of \cref{thm:regret}; the EMA is simply the standard,
reactive default. Deployment can pick either without risking divergence.
