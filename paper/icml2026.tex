...
convergence, guarantee). A dynamic-regret analysis---attaining the per-regime rate
...
see text). The reported SN-OMD row uses the winsorized-EMA tracker; a slow block-median
tracker reaches $R^2\approx0.38$ at $\mathrm{lr}{=}2$ (\cref{sec:tracker}), so this row is
\emph{conservative}.}
\label{tab:jane}
\centering
\begin{small}
...
the winsorized-EMA default at the same rate) and at its tuned rate is \emph{more} accurate
than the EMA ($R^2\approx0.38$ vs.\ $0.28$). We stress-tested this flattering gap rather
than banking it: it is not a boundary artifact---block $R^2$ peaks at the interior
$\mathrm{lr}{=}2$, not at the edge of the extended sweep---but it is high-variance, holding
in $8$ of $10$ contiguous folds with a per-fold gap of $0.16\pm0.37$. Past the tuned range
both trackers eventually diverge (at $\mathrm{lr}{=}10$, peak loss $\approx30$ for the block
tracker vs.\ $\approx230$ for the EMA), so ``does not destabilize'' holds only within
tuning, not everywhere; within it the block tracker degrades far more gracefully.
...
\paragraph{Which tracker to deploy.} Both are usable within tuning; they trade reactivity
against upward variation, and we do \emph{not} claim one is uniformly more accurate. On our
stream the slow regime-timescale (block-median, $B{=}10^4\approx$ one day) tracker is in
fact \emph{more} accurate than the winsorized EMA at the tuned rate ($R^2\approx0.38$ vs.\
$0.28$ at $\mathrm{lr}{=}2$; \cref{sec:tracker}), it degrades far more gracefully as the
rate is pushed past tuning (peak rolling loss $7.9$ vs.\ $54$ at $\mathrm{lr}{=}5$), and it
minimizes the $W_s$ constant of \cref{thm:regret}. We treated this flattering comparison
with the suspicion it deserves: the block advantage is not a sweep-edge artifact (block
$R^2$ peaks at the \emph{interior} $\mathrm{lr}{=}2$) but it is high-variance ($8/10$
contiguous folds, per-fold gap $0.16\pm0.37$), and at $\mathrm{lr}{=}10$ \emph{both}
trackers diverge, so ``pick either without risking divergence'' holds only within the tuned
range. We therefore keep the winsorized EMA as the reported default---it is the standard
reactive choice and it makes \cref{tab:jane} \emph{conservative} rather than optimistic (a
better tracker exists that we do not headline).
