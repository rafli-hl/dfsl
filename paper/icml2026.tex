...
it is \emph{conditionally removable}: the \emph{causally scale-normalized} gradient, the
quantity a learner actually acts on, has a markedly lighter tail
($\hat\alpha\approx2.9$--$3.7$, estimator-dependent). This removability is a property of
\emph{serial dependence}, not of the marginal: an order-\emph{shuffle} that preserves the
marginal exactly destroys it, and the same causal normalization is tail-index
\emph{preserving} on iid streams of known index---so the lightening is neither an estimator
artifact nor a restatement of the pooled tail. The heavy marginal itself is a
volatility-clustering effect: a \emph{bounded} scale drift cannot move a tail index
\citep{breiman1965}, but a predictable scale with a heavy-tailed conditional law can (the
Kesten/GARCH mechanism; \citealt{kesten1973}), and a GARCH surrogate with light innovations
reproduces both $\hat\alpha\approx2.4$ and its removal. What a learner faces is thus the
\emph{predictable} part of the scale, not an intrinsically heavy conditional tail. In this regime plain online
...
factor; we lead instead with the empirically-testable mechanism. Empirically, a whole
family of \emph{bounded scale-free} updates---SN-OMD, classical normalized-GD, a
constant-threshold clip, and AdaGrad-Norm---stays bounded and accurate across a wide
learning-rate range where OGD and scale-dependent clipping diverge, and on this data none
dominates: the best per-protocol accuracy belongs to the constant clip and AdaGrad-Norm,
not to SN-OMD. What singles out SN-OMD is not accuracy but that its scale is
\emph{predictable}, which is exactly what the dynamic-regret guarantee requires; the cap
additionally earns an accuracy optimum where the normalized tail stays heavy.
\end{abstract}
...
\emph{predictable} part of a nonstationary scale, leaving only a mild residual tail. The
...
\begin{itemize}
  ...
  surrogate with light innovations reproduces both the tail and its removal. The effect
  recurs, in weaker form, on image-model training gradients. This is the paper's central
  finding.
  ...
  Under causal standardization SN-OMD stays bounded and accurate (one of several
  co-leading bounded scale-free methods, not the single best; \cref{tab:jane}) where OGD and
...
\subsection{A budget heuristic for dynamic regret under drift}
\label{sec:lower}
...
zero-mean $R^2$. The stability sweeps (\cref{tab:jane},~\cref{fig:main}) stream a
continuous $150{,}000$-row slice (the first ${\sim}20$ trading days). We evaluate
\emph{progressive online} prediction on the historical
...
learner-independent (gradients at the fixed predictor $w^\star$): the pooled index from
${\sim}200{,}000$ gradients, the daily-drift index from the full record.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig1_problem.png}
\caption{\textbf{The pooled tail is real, but conditionally removable.} (a) The
...
noisy. Dividing by a \emph{predictable} causally-tracked scale lightens the tail to
$\hat\alpha\approx2.9$--$3.7$ (estimator-dependent); a marginal-preserving order-shuffle
leaves the pooled index unchanged (an order-statistic identity) yet abolishes the
lightening, so what the tracker removes is serial dependence, not the marginal
...
$\approx2.9$ (trailing median), i.e.\ toward finite higher moments. What makes it removable
is \emph{serial dependence}: the gradient magnitude is strongly clustered---rank (Spearman)
...
component. Three controls (\cref{fig:nullcontrol}; released \texttt{research\_null\_normalization.py}) pin this
...
\emph{predictable} part of the scale, leaving only a mild residual heavy tail. (Per-day $\hat\alpha$ dips below $2$ on a minority of days, but
...
operation lightens the real gradient tail ($2.43\!\to\!3.73$) and a GARCH surrogate with
light innovations ($\approx2.3\!\to\!3.9$--$4.5$), but leaves an order-\emph{shuffle} of the
real series, iid heavy streams of known index, and a bounded $6\times$ drift essentially
unchanged ($\Delta\!\approx\!0$)---so what is removed is serial dependence, not the marginal,
and it is not an estimator artifact. (b) Rank autocorrelation of $\norm{\gt}$ is
slowly-decaying (the volatility clustering the tracker exploits) and vanishes under
shuffling.}
...
learning-rate drops, not from within-phase drift. This inflation of the pooled tail by
scale nonstationarity thus recurs on training gradients, but only in this weaker,
transient form (a finite-sample mixture appearance, not a genuine conditional tail); the genuine
within-regime version---confirmed by the shuffle and iid-null controls and a
feature--residual interaction---we establish only on the market data.
...
(bounded to $\mathrm{lr}=2$), uncapped scale-adaptive OGD (to $\mathrm{lr}=1$), and two
further bounded scale-free baselines, a constant-threshold fixed-$\tau$ clip (to
$\mathrm{lr}\approx0.2$) and AdaGrad-Norm (to $\mathrm{lr}=10$)---reach
$R^2\approx0.08$--$0.38$ across the two protocols. Per-timestep batching lifts OGD from
...
\cref{tab:jane}'s honest reading \emph{is} the point, not a caveat. A whole
\emph{family} of bounded scale-free updates---SN-OMD, normalized-GD, a constant-threshold
fixed-$\tau$ clip, and AdaGrad-Norm---closes the accuracy gap to the divergent baselines,
and on this data \emph{none dominates}: SN-OMD ties normalized-GD ($0.29/0.31$ vs.\
$0.20/0.32$ per-row/per-timestep), while the two most accurate entries are the fixed-$\tau$
clip (per-step $0.382$) and AdaGrad-Norm (per-row $0.329$), \emph{neither} of which uses a
tracked scale or a cap. So it is \emph{normalization in general}---any bounded scale-free
step---that earns the gain; SN-OMD is one instance of this family, not its best member
here. The family is not homogeneous, though: stable learning-rate ranges span two orders
of magnitude (fixed-$\tau$ to $\mathrm{lr}\!\approx\!0.2$, SN-OMD and normalized-GD to $2$,
...
What singles out SN-OMD is therefore narrower than accuracy, and we state it as such.
\emph{(i) Predictability:} its scale $s_t$ is a \emph{predictable} tracker, which is what
makes the dynamic-regret bound (\cref{thm:regret}) go through---normalized-GD's
instantaneous $1/\norm{\gt}$ is not predictable and discards magnitude every round; the
payoff is the (per-regime) bound, not a measured accuracy edge. \emph{(ii) The cap:} a
finite $M$ bounds the step directly, but non-divergence is \emph{not} unique to it---
AdaGrad-Norm stays bounded to a \emph{wider} $\mathrm{lr}\le10$ with no cap at all. What
the cap distinctly buys is an interior \emph{accuracy} optimum when the \emph{normalized}
tail is genuinely heavy, truncating it while preserving bulk magnitude
(\cref{sec:msweep}); on Jane that tail is only mildly heavy ($\hat\alpha\approx3.7$), so
the optimum is flat and SN-OMD, normalized-GD, and the uncapped end are close. One
observation we flag rather than bury: SN-OMD's block-bootstrap standard errors
($\pm.079/.056$) run ${\approx}3\times$ normalized-GD's ($\pm.024/.021$)---its
block-to-block $R^2$ is markedly more variable---which we have not traced to a cause and
suspect reflects the winsorized-EMA tracker's spike response (\cref{sec:tracker}).
...
$0.3$; SN-OMD and normalized-GD are comparable here (the per-row point estimate is
slightly higher for SN-OMD but their block-bootstrap intervals overlap; \cref{tab:jane}), and
uncapped scale-adaptive OGD diverges at large rates. (b) Peak rolling loss: OGD explodes
early and uncapped scale-adaptive OGD at large rates ($10^2$--$10^{30}$); normalized-GD's
peak loss stays $\approx5$ across the range and SN-OMD's stays single-digit, rising to
$\approx8$ at its top stable rate $\mathrm{lr}{=}2$.}
\label{fig:main}
\end{figure*}
...
largest stable learning rate; each $R^2$ carries $\pm$ one circular block-bootstrap
standard error (block $\approx$ one trading day, $4000$ resamples). Scale-\emph{dependent} methods (top) are fragile
...
here; a finite cap prevents the uncapped endpoint's divergence at $\mathrm{lr}\ge2$, though
non-divergence is not unique to it (cf.\ AdaGrad-Norm below). Two
further scale-free bounded baselines---a constant-threshold \emph{fixed-$\tau$ clip}
($\tau{=}20$) and \emph{AdaGrad-Norm}---both satisfy \cref{thm:stability} and are
accuracy-competitive, each the most accurate entry on one protocol (fixed-$\tau$ per-step
$0.382$, AdaGrad-Norm per-row $0.329$); this supports the thesis that \emph{bounded
scale-free} updating, not SN-OMD specifically, is what earns the accuracy over the
divergent baselines, SN-OMD's distinct contributions being its \emph{predictable} tracker
(\cref{thm:regret}) and a cap that earns an accuracy optimum under heavy normalized tails
(\cref{sec:msweep}). The constant threshold cannot track
the $\sim6\times$ scale drift, so its peak loss still grows with the rate (usefully stable
only to $\mathrm{lr}\approx0.2$), whereas AdaGrad-Norm's monotone accumulator keeps its
step $\le\eta$. $R^2$ is not comparable to competition scores (different protocol,
see text). The reported SN-OMD row uses the winsorized-EMA tracker; an alternative
block-median tracker changes the per-row number but not per-step, and not significantly
against this row (its block-bootstrap interval straddles zero; \cref{sec:tracker}).}
\label{tab:jane}
\centering
\begin{small}
...
Normalized-GD ($M\!\to\!0$)          & $0.197{\scriptscriptstyle\pm.024}$ & $0.316{\scriptscriptstyle\pm.021}$ & $\le 2$ \\
...
and $M\to\infty$ is uncapped \emph{scale-adaptive OGD}. Every \emph{finite}-$M$ member
satisfies \cref{thm:stability} ($\norm{\hat g_t}\le M$)---the normalized-GD endpoint
included, its step being $M\,\gt/\norm{\gt}$; the uncapped $M\!\to\!\infty$ endpoint is the
lone exception ($\norm{\hat g_t}=\norm{\gt}/s_t$ is unbounded) and is exactly the member
that diverges (\cref{tab:jane}). So against the \emph{other} stable member, normalized-GD,
non-divergence is not what singles out SN-OMD; the point is that a finite $M$
...
there is \emph{no} meaningful interior accuracy optimum: the three cap-family members
(normalized-GD, SN-OMD, and the uncapped endpoint) are comparable (\cref{tab:jane}). Same mechanism, both
...
per-row}: $R^2=0.38$ vs.\ $0.197$ at each method's tuned rate, an aggregate gap of $+0.18$
whose $95\%$ circular block-bootstrap interval (the \emph{same} one-day blocks as
\cref{tab:jane}) is $[+0.16,+0.21]$ and excludes zero, so the ``ties normalized-GD''
concession (\cref{tab:jane}) is \emph{over-generous for this configuration} on the per-row
protocol. \emph{(ii) But the advantage is protocol-specific}: under the honest batched
(per-timestep) protocol it evaporates---block $0.351$ vs.\ normalized-GD $0.316$, gap
$+0.035$ with interval $[-0.05,+0.12]$ straddling zero---so the headline tie still stands
where it is measured most conservatively. \emph{(iii) The edge over the EMA \emph{default}
is not significant} either way (per-row gap $+0.10$, interval $[-0.04,+0.23]$). We use the
same block bootstrap throughout, in place of a paired-fold $t$-test, since the aggregate
$R^2$ is a ratio of sums and its fold-mean is not the aggregate. Past the tuned range both trackers
...
robustness to a drifting scale. The answer splits by whether the preconditioner is
\emph{monotone}. For the \emph{EMA-denominator} members (RMSProp, Adam) it is no: their
preconditioner divides by $\sqrt{\mathrm{EMA}(\gt^2)}$, which \emph{itself} spikes with
a heavy-tailed gradient (the same round enters numerator and denominator), and with no
scale-\emph{free} cap a single extreme gradient still yields an unbounded step---so
adaptive rescaling alone does not prevent the divergence documented in
\cref{sec:experiments}. \emph{AdaGrad-Norm}, however, divides by the \emph{monotone}
accumulator $\sqrt{\sum_s\norm{g_s}^2}$, which bounds its step by $\eta$ and so does not
diverge; run head-to-head on the Jane stream (\cref{tab:jane}) it stays bounded and is
accuracy-competitive with SN-OMD (indeed best per-row), placing it with the scale-free
bounded family rather than the divergent one. What it lacks relative to SN-OMD is a
\emph{predictable} scale (its denominator depends on the current round) and the
accompanying dynamic-regret guarantee (\cref{thm:regret}). Our unconditional stability
(\cref{thm:stability}) still comes precisely from capping the \emph{normalized} gradient
at a constant $M$.
...
heavy-tailed---a \emph{volatility-clustering} effect of a heavy-tailed conditional scale
\citep{kesten1973}, not something the bounded drift itself can produce
\citep{breiman1965}---while the causally normalized gradient is much lighter. A budget
heuristic (\cref{sec:lower}) motivates why a moving comparator under such drift is hard (we do not lean on it: its
...
iterates provably cannot diverge---at most $O(\sqrt t)$ for any learning rate---and it
attains the worst-tail rate $T^{1/p}$ \emph{per regime}. The scale drift enters as $\sqrt{W_s}$, and $W_s$ grows
...
tracker shrinks the constant but not the exponent (\cref{sec:tracker}). Empirically SN-OMD is one member of a broader \emph{bounded scale-free} family whose
iterates stay bounded across the whole tested learning-rate range where OGD and
scale-dependent clipping diverge---degrading gracefully past the tuned range rather than
blowing up (\cref{sec:tracker}). On this stream no member dominates: SN-OMD ties classical
normalized-GD, and a constant clip and AdaGrad-Norm each lead one protocol
(\cref{tab:jane}). What distinguishes SN-OMD is not accuracy but its \emph{predictable}
tracker---which the dynamic-regret guarantee requires---and a cap that earns an accuracy
optimum only when the normalized tail stays heavy (\cref{sec:msweep}).
...
its stable learning-rate range is far wider than the scale-dependent baselines' and it
degrades gracefully.
The proven $W_s=O(V_\sigma^+)$ guarantee is for the two-timescale envelope tracker; the
deployed default is a winsorized EMA and the most accurate per-row on our stream is a
block median, neither of which \cref{prop:tracker} covers (\cref{sec:tracker}). Closing
that gap with a single tracker that is both provably bounded-variation and empirically
best is open, as is making the envelope's release rate $\rho$ adaptive to an unknown
regime length.
So the dynamic-regret bound is proven only for the envelope variant
(\cref{ass:moment,ass:track}); the deployed EMA's high-probability regret under drift is
supported empirically. Empirically, three gaps remain: the EMA-denominator adaptive optimizers (RMSProp, Adam)
are argued to diverge but not yet run head-to-head, though the monotone AdaGrad-Norm and a
constant-threshold clip now are (\cref{sec:related,tab:jane}); evaluation centers on
accuracy and stability rather than mechanism-level diagnostics; and validation beyond our
two domains is warranted.

\section*{Software and Data}
The released code provides the \texttt{ScaleNormalizedOGD} learner as a library class,
the scale-tracker implementations used in the experiments (winsorized EMA, two-timescale
envelope, and block median) as experiment scripts, the configs, and the scripts that
reproduce every figure and number. The primary data is the public Jane Street Real-Time
Market Data Forecasting competition; the second-domain gradient norms are logged from a
small CNN trained on MNIST (\cref{sec:experiments}).
...
block median} ($B{=}10^4\approx$ one day) is the most accurate \emph{per-row} on our stream
(per-timestep the constant-$\tau$ clip edges it, $0.382$ vs.\ $0.351$; \cref{tab:jane}) yet
...
On accuracy we are deliberately careful, because the numbers are easy to over-read (block
bootstrap of the aggregate gap, each method at its tuned rate; \cref{sec:tracker}). The
block tracker \emph{does} beat the normalized-GD baseline per-row---$0.38$ vs.\ $0.197$,
gap $+0.18$, $95\%$ CI $[+0.16,+0.21]$---but that edge is \emph{protocol-specific}: under
the batched per-timestep protocol it vanishes ($0.351$ vs.\ $0.316$, gap $+0.035$, CI
$[-0.05,+0.12]$). The mechanism is substitution,
not coincidence: per-timestep batching averages the $\sim\!9$ correlated symbols in each
$(\text{date},\text{time})$ group, and that averaging is itself the variance reduction the
smooth block scale supplies per-row, so the two do not compound---when the protocol smooths
the update, a smoother tracker adds nothing. The same averaging is what lifts
normalized-GD ($0.197\to0.316$, it had no such smoothing per-row) while, uncapped, it
\emph{enlarges} the untruncated step and halves scale-adaptive OGD ($0.162\to0.080$,
\cref{sec:experiments}); the block tracker's edge sitting exactly where the protocol lacks
the smoothing is the same story from the tracker side. Against the EMA \emph{default}
its edge is \emph{not} significant on either protocol (per-row gap $+0.10$, CI
$[-0.04,+0.23]$). So we do \emph{not} claim the block tracker is uniformly more accurate, and
we keep the winsorized EMA as the reported default: it is the standard reactive choice, and
the block tracker does not beat it significantly. Past the tuned range every tracker
