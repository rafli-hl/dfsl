...
additionally buys non-divergence at large steps.
\end{abstract}
...
Kesten/GARCH mechanism, \citealt{kesten1973}; equivalently the subordination of
...
of it. The same holds for two further scale-free bounded baselines---a constant-threshold
fixed-$\tau$ clip and AdaGrad-Norm---which are likewise competitive here (each the most
accurate entry on one protocol; \cref{tab:jane}), confirming that it is bounded scale-free
updating in general, not SN-OMD in particular, that closes the accuracy gap to the
divergent methods. SN-OMD's specific contributions lie elsewhere. First, its scale is a
...
slightly higher for SN-OMD but within fold noise, $t\approx0.1$; \cref{tab:jane}), and
...
here; the cap's role is non-divergence (uncapped diverges at $\mathrm{lr}\ge2$). Two
...
(\cref{thm:regret}) and non-divergence at large steps. The constant threshold cannot track
...
tracker shrinks the constant but not the exponent (\cref{sec:tracker}). Empirically SN-OMD's
iterates stay bounded across the whole tested learning-rate range where OGD and
scale-dependent clipping diverge---accuracy degrading gracefully past the tuned range
rather than blowing up (\cref{sec:tracker})---and it matches the classical normalized-GD
in accuracy while adding a regret guarantee and non-divergence at large steps.
