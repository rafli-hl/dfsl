...
(clipping $\norm{\gt}$ at $s_t$; the lag mechanism is the \cref{sec:tracker} clip-rate/dynamic-range
tension), a lagging $s_t$ clips hardest precisely when the scale jumps up---the stale
threshold throws away the informative post-onset gradients. In the same sweep the threshold
use never leaves $R^2\approx0.003$ at \emph{any} window---scale-dependent clipping is stable
only at a tiny step size, so it is dominated by that first-order tax rather than by the
window, and the normalizer beats it at every tracker length (we therefore do \emph{not} claim
a clean window-monotone threshold curve, only that the threshold is uniformly worse). Long
windows are thus fine for normalizing and never help thresholding; the one-day block tracker
is a normalizer, so no contradiction arises.
