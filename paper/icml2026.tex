...
calibrated to the real raw index the \emph{causal} tracker recovers $3.9$--$4.5$ (matching
the real $3.73$) against a true-$\sigma_t$ oracle floor of $\hat\alpha\!\approx\!9$---so the
residual $\hat\alpha\!\approx\!4$ is the signature of \emph{causally} tracking a clustered
scale, reproduced with known ground truth.}
\label{tab:residual}
\centering
\begin{small}
...
positive at every threshold and estimator, and order-one ($+1.0$ to $+1.5$) for the deployed
tracker (\cref{tab:residual}); isolating causality---the \emph{same} estimator run
non-causally---adds only $\sim\!+0.3$, so the residual $\hat\alpha\!\approx\!4$ is not a
tracker-lag artifact but the signature of causally tracking a clustered scale. What makes it removable is \emph{serial dependence}: the gradient magnitude is strongly clustered---rank (Spearman)
...
our two domains is warranted. Finally, the deployed normalized tail sits at
$\hat\alpha\!\approx\!3.7$--$4$ ($p>2$, finite variance; \cref{tab:residual}), so the
$p\in(1,2]$ machinery is a worst-case guarantee, not a description of the residual faced on
these streams.
