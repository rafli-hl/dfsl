...
family is taken up in \cref{sec:experiments}. Because the features are bounded, the
loss-based divergence reported there moves in lockstep with $\norm{w_t}$; \cref{app:iternorm}
logs $\norm{w_t}$ directly and finds it never leaves the $\norm{w_1}+2M\eta\sqrt{t}$
envelope, whereas the uncapped ($M\!\to\!\infty$) endpoint and OGD breach it by many orders.
...
${\sim}200{,}000$ gradients, the daily-drift index from the full record. We characterize the tail at the fixed
$w^\star$ so it is a property of the data, not of any learner; we do not separately track
it along the online trajectory $\{w_t\}$, whose norm the stability guarantee
(\cref{thm:stability}), not a tail assumption, governs.
...
single-day Hill estimates are noisy and we do not rely on them.) The per-regime budget of
\cref{sec:lower} is dominated by exactly these pooled-$\alpha\!<\!2$ days, which largely
disappear once the gradient is normalized (the tail the learner sees has $p\approx2$); we treat
it as qualitative motivation only, not a measured quantity.
...
windows; the uncapped endpoint and plain OGD do not. \emph{No single member dominates across both
protocols:} per-row the block-median SN-OMD and the normalize-and-clip method of
\citet{cutkoskymehta2021} \emph{co-lead} in a statistical tie ($0.29\!\pm\!.07$ vs.\
$0.29\!\pm\!.04$; paired $+0.004$, CI $[-0.014,+0.023]$; \cref{app:tracker}), both clear of the
rest; per-step no member leads decisively. Rates are tuned on window~1 over a grid with all
optima \emph{interior} (\cref{app:grid}); CM at $\beta{=}0$ \emph{is} normalized-GD (bit-identical
per-step cell, not a duplicated row).}
...
develop (i)~the \emph{cap frontier}---SN-OMD interpolates normalized-GD ($M\to0$) and uncapped
scale-adaptive OGD ($M\to\infty$), every finite-$M$ member is stable (\cref{thm:stability}) and
only the uncapped endpoint diverges, and an $M$-sweep gives a \emph{sharp} interior accuracy
optimum under an imposed heavy tail but a \emph{broad} one on Jane's milder residual
(\cref{tab:residual}); and (ii)~the empirical discharge of \cref{ass:track}, where a
two-timescale peak-hold envelope holds the stability-critical lower bracket at $100\%$ of rounds
with $W_s\approx6.6\,V_\sigma^+$ growing \emph{linearly} in $T$ (so \cref{thm:regret}'s $T^{1/p}$
is per-regime), the per-row accuracy-optimal tracker being a block median that co-leads across
the ten frozen windows (\cref{tab:replication}).
...
in-expectation) guarantee. A concurrent line sharpens the normalization-versus-clipping question for \emph{offline
non-convex} SGD, proving normalization---with or without clipping---suffices for convergence
under only a bounded $p$-th moment \citep{hubler2025clipping,sun2024normalization}; we instead
...
also rescale by a running second moment; whether they handle a drifting scale splits on whether
the preconditioner is \emph{monotone}. The \emph{EMA-denominator} members (RMSProp, Adam) divide
by $\sqrt{\mathrm{EMA}(\gt^2)}$, which itself spikes with a heavy-tailed gradient (numerator and
denominator share the round), so with no scale-\emph{free} cap a single extreme gradient still
yields an unbounded step---the divergence of \cref{sec:experiments}. \emph{AdaGrad-Norm} divides
by the \emph{monotone} accumulator $\sqrt{\sum_s\norm{g_s}^2}$, bounding its step by $\eta$;
head-to-head on Jane it stays bounded and accuracy-competitive ($0.20\!\pm\!.07$ per-row;
\cref{tab:replication}), placing it with the bounded scale-free family---what it lacks is a
\emph{predictable} scale and the dynamic-regret guarantee (\cref{thm:regret}).
...
diverge, and what singles out SN-OMD is a \emph{predictable} tracker (required by the
dynamic-regret guarantee)---not an accuracy win: with the block-median scale it co-leads per-row.

\paragraph{Limitations.} SN-OMD is not fully tuning-free (setting-dependent rate; untuned $M$,
decay, winsorization). The $W_s=O(V_\sigma^+)$ guarantee covers only the two-timescale envelope
(\cref{ass:track}), not the deployed EMA/block-median trackers (empirical only,
\cref{sec:tracker}); RMSProp/Adam are argued to diverge but not run head-to-head; and validation
beyond financial markets is warranted. Finally, the normalized residual tail is dominated by
...
public competition data (Kaggle-gated), as disclosed in \cref{sec:experiments}.

\section*{Use of Large Language Models}
% Please confirm this matches your actual LLM use before submitting; ICLR 2027 requires an
% accurate disclosure for research subtasks (see the ICLR AI Policy for Authors).
...
cross-checking; and for an independent audit pass over the manuscript that re-derived the
theorems, checked paper--code consistency, and assisted the preparation of the submission in the
ICLR format. LLMs were \emph{not} used to generate experimental data and did not originate the
...
\label{app:tables}
Collected here for space: the residual tail-index decomposition (\cref{tab:residual}), the
leakage controls (\cref{tab:leakage}), the single-window tuned comparison (\cref{tab:jane}), and
