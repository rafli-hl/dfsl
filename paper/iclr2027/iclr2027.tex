...
$O(\sqrt t)$ for any rate or realization---a stability, not a convergence, guarantee. Scale-\emph{invariance} further buys a \emph{transferable} tuning: a degree-zero step map's
stable rates do not move with the gradient scale, predicting the $9/10$-vs-$0/10$ divergence
split we measure. Empirically a whole family of \emph{bounded scale-free} updates stays bounded and
...
replicate on a second, independent market. Two limits we state rather than bury. The
dynamic-regret bound we prove, at rate $T^{1/p}$ ($p\in(1,2]$), is \emph{permanently}
per-regime: all three routes that would globalize it are measured shut.
...
\begin{itemize}
  ...
  baseline rather than beating it (\cref{tab:replication}). What scale-invariance adds is a
  \emph{transferable} tuning (\cref{thm:transfer}), predicting the $9/10$-vs-$0/10$ split we
  measure; the regret bound (\cref{thm:regret}) is demoted. The study is protocol-careful: the result is invariant to
...
do not treat as free: \cref{prop:tracker} discharges it under a drift model whose domination
condition we then \emph{measure to fail} on our own data (\cref{app:regimecount}).
...
norms) and takes a scale-invariant, constant-capped step (\cref{alg:snomd}), where
$\clip(v,M)=v\cdot\min\{1,M/\norm v\}$. Under the Euclidean regularizer used throughout, the
mirror step is exactly the projected gradient step shown there.
...
\paragraph{The scale tracker.} $s_t$ may be any predictable robust tracker, and the choice
is where this paper's honest gap sits. We report three. A \emph{winsorized EMA} of
$\norm{\gt}$ reacts quickly and cheaply, and is the default we deploy. A \emph{two-timescale
peak-hold envelope} rises on a short robust median and releases only geometrically; it is the
one tracker for which we can \emph{prove} the condition the regret bound needs. A \emph{block
median} over ${\approx}$one-day blocks is the most accurate per-row across ten frozen windows.
The trackers we would deploy are not the one we can bound, and \cref{sec:tracker} states that
...
\begin{proposition}[Bounded iterates, unconditional]
\label{thm:stability}
For any inputs, $\norm{\hat g_t}\le M$ deterministically. Hence for any learning rate, scale
drift, or realization, $\norm{w_t}\le D$ on a bounded domain $\mathcal W$, and
$\norm{w_t}\le\norm{w_1}+2M\eta\sqrt t$ on $\R^d$ under the step $\eta/\sqrt t$.
\end{proposition}
This is a \emph{stability} guarantee---bounded iterates, or $O(\sqrt t)$ growth---and not by
itself convergence; it uses no moment assumption at all. That is the crucial contrast with
scale-\emph{dependent} clipping, whose per-step move $\propto s_t$ is bounded only by the
drifting scale.

It is also a property of a \emph{family} rather than of SN-OMD: any update whose step is capped
by a scale-free constant satisfies it, including normalized-GD, a constant-threshold
(fixed-$\tau$) clip, and AdaGrad-Norm (\cref{tab:jane}). So it separates the bounded scale-free
family from the scale-dependent methods, and what singles out SN-OMD \emph{within} that family
is taken up in \cref{sec:experiments}. Because the features are bounded, the loss-based
divergence reported there moves in lockstep with $\norm{w_t}$, which \cref{app:iternorm} logs
directly and finds inside the bound on every run.

\paragraph{What scale-invariance buys: a tuned rate that transfers.} \cref{thm:stability}
concerns one stream; what governs behaviour \emph{across} streams of differing scale is how the
step map responds to rescaling them. Write $w_{t+1}=w_t-\eta_t\Phi_t(g_1,\dots,g_t)$, and let
$S_\lambda$ scale every $g_t$ by $\lambda>0$.
\begin{theorem}[Tuning transfer]
\label{thm:transfer}
If $\Phi$ is positively homogeneous of degree $0$, the trajectory on $S_\lambda$ at rate $\eta$
is that on $S_1$ at rate $\eta$: the stable-rate set does not move with $\lambda$. If $\Phi$ is
degree $1$, it is that on $S_1$ at rate $\lambda\eta$: the stable-rate set contracts as
$1/\lambda$, so no fixed rate is stable uniformly in $\lambda$.
\end{theorem}
SN-OMD, normalized-GD and AdaGrad-Norm are degree $0$; OGD and clipping to $\tau_t=c\,s_t$ are
degree $1$ (\cref{app:family}, with the proof). Frozen from window~1 across the ten windows, OGD
diverges $9/10$ per-step (\cref{tab:replication}) as do both robust clippers
(\cref{app:family}), while every degree-$0$ method diverges $0/10$. What transfers is
\emph{stability}, not accuracy: a degree-$0$ method keeps its bound and can still lose accuracy,
as normalized-GD does (\cref{app:grid}).

\paragraph{Why the regret bound is not the headline.} Predictability of $s_t$ also buys a
high-probability \emph{per-regime} dynamic-regret guarantee,
$\tilde O((\sup_t\sigma_t)(D+\sqrt{D P_T})T^{1/p}\log(1/\delta))$, stated with explicit
constants as \cref{thm:regret}. It sits there because all three routes that would make it a
headline are measured shut. Its carrier $W_s=s_1+\sum_t(s_t-s_{t-1})_+$ grows linearly in $T$
(\cref{sec:tracker}), so the exponent becomes $T^{1/p+1/2}$---vacuous at $p{=}2$, where the
measured tail index puts us; the switching bound that would lift the per-regime restriction
needs a \emph{sublinear} regime count, measured at least linear; and \cref{prop:tracker}, which
discharges \cref{ass:track}, pays a term dominated only when $NW\ll T^{1/p}$, which fails on
both factors (\cref{app:regimecount}). It is also per \emph{fixed predictable} comparator, not
the per-round minimizer (\cref{rmk:scope}). The obstructions, not modesty, license calling the
per-regime restriction permanent.
...
(\cref{tab:jane,fig:main}), the methods split by whether their step \emph{bound} moves with the
gradient \emph{scale}. \emph{Scale-dependent} methods---plain OGD and the robust clippers
(AdaptiveClip, RobustOMD), whose threshold $\tau_t=c\,s_t$ rescales with the regime---all carry
such a bound, and all are fragile: stable only to $\mathrm{lr}\approx10^{-2}$ and reaching best
...
distinct: \cref{thm:regret} needs predictability, which says nothing about how accurate any
...
re-run at the matched budget, puts them at $-0.005$, CI $[-0.036,+0.025]$, $6/10$)---both
clear of the rest (fixed-$\tau$ $0.21$,
...
gets over $(\mathrm{lr},\tau)$, the threshold-free methods over $\mathrm{lr}$ alone---their
whole parameter space---and CM over $(\mathrm{lr},\tau,\beta)$, which is \emph{three} and the
largest budget in the table (\cref{app:grid}). Every selected optimum is interior except the per-step SN-OMD row, which
...
\paragraph{Further experiments.} Two supporting studies are deferred to
\cref{app:further}: the \emph{cap frontier}, an $M$-sweep across the interpolation from
normalized-GD to the uncapped endpoint (\cref{sec:msweep}); and the empirical discharge of
\cref{ass:track} (\cref{sec:tracker}).

\section{Related Work}
\label{sec:related}

Three literatures meet here. The volatility literature has the \emph{mechanism}, but for asset
returns rather than gradients; robust estimation and clipping have the \emph{primitive}, but
take the gradient scale as given; distribution-free online learning has the \emph{guarantees},
but not against a drifting scale and a moving comparator. That last gap is our setting, and
unless noted below it is what separates this work from each line.
...
\emph{predictable} tracked scale rather than clipping the current gradient.

We ran their method head-to-head across the ten frozen windows, tuned over its momentum
coefficient. It stays bounded ($0/10$) and \emph{co-leads} per-row, statistically tied with the
block-median SN-OMD (\cref{tab:replication}). That co-lead is bought by \emph{momentum}, not the
clip---at $\beta{=}0$ their method reduces exactly to normalized-GD, and the clip sits idle on
its plateau (decomposition in \cref{app:tracker}). So momentum-normalized SGD, not
clip-plus-normalization, ties the tracked scale, and neither leads both protocols; what the
\emph{predictable} scale adds is the guarantee, not a measured accuracy edge.
...
a Hedge meta-algorithm and no clipping; we differ in the predictable tracker and hard cap, and
in giving a \emph{high-probability} rather than in-expectation guarantee.
...
under only a bounded $p$-th moment
\citep{liuzhou2025nonconvex,hubler2025clipping,sun2024normalization}; that line is offline, and
we add the measurement that the gradient tail is itself a predictable-scale artifact.
...
remove knowledge of the regime length. Scale-freeness there is invariance to one \emph{global}
rescaling of the gradients; the drift we face is \emph{within} a stream, which is what the
predictable tracker is for.
...
The difficulty on real streams is a nonstationary gradient scale, and what the learner faces
is its \emph{predictable} part. That reframing is what the paper contributes: it makes a heavy
gradient tail a property to be \emph{removed} rather than survived, it makes non-divergence
something a scale-free cap can guarantee outright, it makes a tuned rate \emph{transfer} across
streams of differing scale (\cref{thm:transfer}), and it makes the accuracy question one about
the whole bounded scale-free family rather than one member---where our own method ties rather
than wins.
...
markets only. The normalized residual tail is dominated by
\emph{causal-estimation cost} ($\hat\alpha\approx3.73$; \cref{tab:residual}), so with $p>2$ the
cap is for stability alone---and the per-regime restriction on \cref{thm:regret} is not
a gap we expect to close: the switching bound that would remove it needs a sublinear regime
count, and we measure that count to grow linearly (\cref{app:regimecount}). Finally, the
anytime $\eta/\sqrt t$ schedule we deploy is not the
more accurate one: on window~1 at a shared rate grid, SN-OMD scores $0.52$ under a
\emph{constant} rate against $0.25$ under the schedule (\cref{app:adaptive})---what the anytime
$O(\sqrt t)$ envelope costs, and we do not claim it back.
...
counts (e.g.\ the $400$-seed and five-seed runs).
What a reviewer \emph{without} the competition data can still run is the whole Jane code path
on a seeded mini-slice in the same schema (\texttt{research\_jane\_mini.py}): on that stream
the bounded scale-free updates stay bounded at every rate we sweep while the scale-dependent
and uncapped ones diverge, and causal normalization lightens the pooled Hill index from
$2.83$ to $3.75$. Those are not the paper's numbers---the stream's drift is ours to set---but
they exercise the loader, the predictable tracker, the cap and the evaluation, and they
reproduce the qualitative dichotomy the paper's reading rests on. All experiments run on a single commodity
...
figure-and-table suite reproduces without a GPU. It takes about 3~hours of wall-clock to do
so. \texttt{research\_runtime\_suite.py} times a defined set---every script named in this
paper, plus those that generate its tables (29 in all, one fresh interpreter each)---and
records the per-script breakdown; nearly half the total is the two matched-budget ten-window
runs behind \cref{tab:replication}. The part that needs no Kaggle-gated data, which is
everything a reviewer without a competition account can run, takes about 3~minutes.
...
Because the competition data cannot be redistributed, \texttt{research\_jane\_mini.py} ships a
seeded mini-slice in the Jane schema that drives the \emph{same} loader, update harness and
evaluation with no market data at all, and the same script hashes the real canonical slice so a
reviewer holding their own download can confirm it matches ours before spending the compute.
...
\paragraph{Is the one-day bootstrap block long enough (\cref{tab:jane})?} A block should
span the serial dependence it is there to respect, and this stream's rank autocorrelation is
still ${\approx}0.16$ at lag $100$, so one trading day may be too short---which would \emph{understate}
every standard error in \cref{tab:jane}. We re-bootstrapped each row at its own tuned setting
with blocks of one, three, five and ten trading days
(\texttt{research\_block\_length.py}). At the three-day block the standard errors do not move
(ratios $0.88$--$1.18$ across the twelve series), so at that scale the one-day block is not
understating them. Past three days they \emph{shrink}---which is not evidence that the
dependence is absent. The slice is $20$ trading days, so a three-day block leaves 7 blocks per
resample and a ten-day block leaves 2, and a moving-block bootstrap assembled from a handful of
long contiguous stretches collapses toward the original series. Those columns measure the record
length, not the dependence, and settling the question at longer horizons needs a longer slice.
What we can say is that every reading we take from \cref{tab:jane} holds at every block length
tried: the per-step SN-OMD and normalized-GD intervals overlap throughout, SN-OMD and
AdaGrad-Norm overlap per-row throughout, and the separation from scale-dependent OGD is disjoint
throughout. The accuracy claims in any case do not rest on this bootstrap but on the paired
across-window bootstrap of \cref{app:tracker}, which resamples at a granularity that absorbs
within-window dependence outright.
...
Scale-\emph{dependent} methods (top) are fragile---OGD is stable only to
...
(\cref{tab:replication}). $^\dagger$Per-row only; ceiling measured at $[10^{-2},2\!\times\!10^{-2})$
on this stream (\cref{app:family}). $R^2$ is not comparable to competition scores.}
\label{tab:jane}
\centering
\begin{small}
\setlength{\tabcolsep}{3.5pt}
...
AdaptiveClip / RobustOMD$^\dagger$   & $\sim0.01$ & --- & $\le 10^{-2}$ \\
\midrule
...
These are the two studies the main text defers: what the cap $M$ buys as it sweeps between
the two endpoint methods, and how closely a predictable tracker meets \cref{ass:track} on the
real gradient-scale process. The studies the main text does \emph{not} promise---other
streams, other trackers, other grids, other optimizers---are collected in \cref{app:scope}.
...
\subsection{Discharging the scale-tracking assumption: what we measure}
\label{sec:tracker}
...
\emph{empirically}. The analytical route to the same conclusion, \cref{prop:tracker}, is not
free at this drift rate: its exceptional-set term is not dominated here
(\cref{app:regimecount}).
...
bound in the regime count---which \cref{app:regimecount} measures to be unavailable, since
that count grows linearly in $T$ too. The most accurate per-row tracker
on Jane is a block median; across the ten frozen windows it co-leads per-row
($0.28\!\pm\!.09$, tied with a normalize-and-clip baseline; both above normalized-GD's
...
Escaping the resulting $\sqrt T$ drift factor would need a switching bound in the regime
count, which the same measurement rules out: counted at this reliability threshold the regime
count itself grows linearly (\cref{app:regimecount}).}
\label{tab:beta}
\centering
\begin{small}
...
This section states the dynamic-regret guarantee in full: the unsimplified
...
exponent. Escaping the $\sqrt T$ would need a switching bound in the regime count---and on
this process that route is \emph{closed}, not merely unattempted. Such a bound gives
$\tilde O(N^{1-1/p}T^{1/p})$, which improves on $T^{1/p+1/2}$ only when the regime count $N$ is
sublinear in $T$. Counting regimes on the real scale process by the definition used here---a
horizon over which the tracker's upward variation is $O(1)$---gives $N$ growing with exponent
$1.16$--$1.24$ in $T$, across two trackers and a sweep of the per-regime budget, wherever there
are enough segments to fit an exponent at all (\cref{app:regimecount}). At $N=\Theta(T)$ the
switching rate is $\Theta(T)$ for every $p$. So the residual $\sqrt T$ here is a property of the
measured drift, not a gap in this proof.
\end{remark}
...
now visible in the object the theorem bounds. Generated by \texttt{research\_iterate\_norm.py --lr 8}.
...
\subsection{What puts a method in the bounded scale-free family}
\label{app:family}
\cref{thm:stability} is stated for SN-OMD's cap, and \cref{sec:experiments} then places
normalized-GD, a fixed-$\tau$ clip and AdaGrad-Norm in the same family. Two \emph{independent}
properties of the step map $\Phi_t$ in $w_{t+1}=w_t-\eta_t\Phi_t$ are at work, and separating
them explains all three memberships. Write $\Phi_t$ \emph{bounded} if $\norm{\Phi_t}\le B$ for a
constant independent of the stream, and \emph{scale-invariant} if $\Phi_t(\lambda g)=\Phi_t(g)$
for every $\lambda>0$.

Boundedness alone gives stability: $\norm{w_{t+1}-w_t}\le\eta B/\sqrt t$ and
$\sum_{k<t}k^{-1/2}\le2\sqrt t$ reproduce \cref{thm:stability}'s envelope for any such method.
Scale-invariance is a separate matter, and it is what \cref{thm:transfer} governs.

\begin{proof}[Proof of \cref{thm:transfer}]
Both cases are an induction on $t$ from $w_1$, which does not depend on $\lambda$. If $\Phi$ is
degree zero then $\Phi_t(\lambda g_1,\dots,\lambda g_t)=\Phi_t(g_1,\dots,g_t)$, so the update
$w_{t+1}=w_t-\eta_t\Phi_t$ is identical on $S_\lambda$ and on $S_1$ at the same $\eta$. If
$\Phi$ is degree one then $\Phi_t(\lambda g)=\lambda\Phi_t(g)$ and the update is
$w_{t+1}=w_t-\eta_t\lambda\Phi_t(g)$, which is the rate-$\lambda\eta$ update on $S_1$. Hence the
stable-rate set satisfies $H(S_\lambda)=H(S_1)$ in the first case and $H(S_\lambda)=H(S_1)/\lambda$
in the second, and in the second no fixed $\eta$ lies in $H(S_\lambda)$ for every $\lambda>0$.
\end{proof}

\noindent Plain OGD is degree one, and so are the clippers, since $\tau_t=c\,s_t$ scales with
the stream. This is stated for a \emph{given} gradient sequence; under the feedback of a linear
model $g_t$ depends on $w_t$, and we do not claim the induction unchanged there.

Two consequences worth stating. First, a degree-one method can only have its displacement
bounded uniformly in $\lambda$ by a rate carrying a $1/s_t$ factor---so normalization is not one
repair among several. That is why \textbf{AdaGrad-Norm} belongs here despite never clipping: its
step $\eta\gt/\sqrt{\sum_{k\le t}\norm{g_k}^2}$ pairs a degree-one $\Phi$ with a
degree-\emph{minus}-one rate, hence is degree zero overall and bounded by $\eta$. Second, the
fixed-$\tau$ clip is bounded but \emph{not} degree zero, being degree one below the threshold
and degree zero above; it is therefore stable at every scale yet its useful rate does not
transfer---which is exactly \cref{fig:mechanism}(b).

What this does \emph{not} give is a separation: nothing here forces a scale-dependent method to
diverge, only its upper bound to be loose and its tuning not to transfer.

\paragraph{The clippers on the frozen instrument.} A scale-dependent threshold is bounded only
by $c\,s_t$, which is not a stream-independent constant, so the argument above does not cover
the robust clippers---and measurement agrees. Run through the ten-window frozen protocol at the
budget the instrument gives the other one-dimensional methods, AdaptiveClip and RobustOMD tune
on window~1 to $\mathrm{lr}{=}10^{-3}$ with $R^2=0.013$ and $0.014$, and their stability ceiling
sits at $[10^{-2},2\!\times\!10^{-2})$---so \cref{tab:jane}'s $\le\!10^{-2}$ holds on this
stream. Frozen and carried to the other nine windows they diverge $9/10$ per-step, exactly OGD's
count, and per-row stay bounded ($0/10$) but at mean $R^2$ of $-0.007$ and $-0.010$, below OGD
and far below the bounded scale-free pack's $0.14$--$0.21$. This is what degree-one homogeneity
predicts: a rate tuned on one window's scale need not survive a window whose scale differs.
Bounded \emph{in-window} at a small enough rate is not the same property as bounded by a
constant, and only the latter transfers.

\paragraph{What transfers is stability, not accuracy.} The scope is worth stating, because the
two are easy to conflate and \cref{tab:replication} contains both. Degree-zero homogeneity makes
the \emph{set of rates at which the iterate stays bounded} invariant to rescaling the stream; it
says nothing about which rate inside that set is most accurate. Frozen per-step at
$\mathrm{lr}{=}5$, normalized-GD diverges on $0/10$ windows while its $R^2$ runs from $0.37$ down
to $-0.20$---so its stability transferred and its accuracy did not, which is the
overfitting \cref{app:grid} attributes it to and not a failure of the invariance. The clippers
fail the other way, diverging on $9/10$. Degree separates the two modes: degree one loses
stability, degree zero keeps it and can still lose accuracy.
...
\subsection{Discharging the scale-tracking assumption: what we can prove}
\label{app:bracket}
...
Five studies ask whether the reading survives a change of stream, of tracker, of tuning
grid, or of optimizer. Two of them (\cref{sec:synthetic,app:secondmarket}) ask whether the
mechanism appears away from Jane---on a synthetic stream whose tail index we set, and on an
independent market. Two (\cref{app:tracker,app:grid}) ask whether our own choices are
carrying the result: which tracker we run, and whether the tuning grid is wide enough to
have found each method's optimum. The last (\cref{app:adaptive}) runs the adaptive
optimizers rather than arguing about them.
...
factor. This is the picture the measured gradient scale fits \emph{locally}
($\sim\!30\%$ within-day drift, occasional $3\times$ overnight jumps; \cref{sec:experiments}),
but not globally in the sense the proposition needs: at every tracker resolution that holds the
lower bracket, change points arrive at a constant \emph{rate} rather than being rare, so $N$
scales with $T$ (\cref{app:regimecount}).
...
\cref{sec:tracker}. The residual---the post-jump lapse---is an additive term of the form
isolated in \cref{app:reductions}, and making $\rho$ adaptive to an unknown $L$ is exactly
that subsection's strongly-adaptive wrapper.
...
observation. SN-OMD's per-row error bar in \cref{tab:jane} is the \emph{widest} in that
column---$\pm.080$, about twice normalized-GD's $\pm.037$ and $3.3\times$ the tightest bar
there---though it is no wider than normalized-GD's per-step. The replication does \emph{not}
trace that width to the EMA tracker: once the cap is tuned alongside the rate, the EMA is the
\emph{tighter} of the two SN-OMD rows across windows.
...
(\texttt{research\_c13\_matched\_bootstrap.py}, which recomputes
\texttt{research\_tracker\_bootstrap.py}'s statistic on the adopted matched-budget rows).
Resampling at the \emph{window} granularity
...
tracker beats every method \emph{except one}, each with a paired $95\%$ CI clear of
zero---normalized-GD by $+0.103$ ($[+0.058,+0.148]$, $10/10$), AdaGrad-Norm by $+0.084$
($8/10$), the fixed-$\tau$ clip by $+0.075$ ($9/10$), and the EMA default by $+0.046$
($[+0.016,+0.076]$, $8/10$)---but only \emph{ties} the normalize-and-clip Cutkosky--Mehta
baseline ($-0.005$, CI $[-0.036,+0.025]$, $6/10$), which co-leads and is now the nominal
leader. Tuning the cap alongside the rate cut the margin over the EMA by roughly two thirds
(it was $+0.15$ when the cap was pinned at $M{=}5$), which is the same lesson \cref{app:grid}
draws: the spread an earlier draft charged to the tracker belonged to the pinned cap. The paired instrument \emph{supersedes} the single-window block-bootstrap gap of
...
significant gaps intact (EMA $+0.039$, $[+0.010,+0.067]$, $7/9$; normalized-GD $+0.092$,
$9/9$) and the CM tie unchanged ($-0.013$, $[-0.042,+0.017]$, $5/9$). The picture is
\emph{protocol-specific}, and at the matched budget per-step is where the block tracker looks
strongest: it clears zero against the EMA default ($+0.028$, $[+0.017,+0.039]$, $10/10$) and
the fixed-$\tau$ clip ($+0.040$, $10/10$), where the pinned-cap run had tied both, and ties only
AdaGrad-Norm ($+0.054$, $[-0.021,+0.129]$, $7/10$). Two cautions on reading that. The per-step
SN-OMD row is selected at \emph{both} grid edges (\cref{app:grid}), so it is a truncated
comparator rather than a tuned one, and the gap over it is correspondingly a lower bound on how
well a properly interior EMA row would do. And normalized-GD's window-1-optimal per-step rate
overfits (\cref{app:grid}) and collapses across regimes, so the $+0.148$ ($10/10$) over it
measures that collapse rather than the tracker---as it does for the likewise-collapsed CM,
which per-step
...
$\alpha/14$ ($t_9$ critical value $3.91$ instead of $2.26$) changes two verdicts, and at the
matched budget one of them is the \emph{per-row} tracker gap this subsection exists to report:
block over the EMA default, $+0.046$, goes from $[+0.016,+0.076]$ to $[-0.006,+0.098]$ and is no
longer separable from zero. The other is the per-step gap over the uncapped scale-adaptive
endpoint, $+0.147$, becoming $[-0.007,+0.301]$. We state the first plainly rather than lean on
the uncorrected interval: \emph{under correction for the fourteen tests, the block median is not
shown to beat the deployed EMA per-row.} What survives correction is the separation from the
rest of the family---normalized-GD $+0.103$ ($[+0.026,+0.180]$), AdaGrad-Norm $+0.084$
($[+0.017,+0.150]$), the fixed-$\tau$ clip $+0.075$ ($[+0.002,+0.149]$), OGD $+0.281$---together
with the per-step gaps over the EMA ($+0.028$, $[+0.009,+0.047]$) and the clip ($+0.040$,
$[+0.011,+0.070]$). Every tie stays a tie, the Cutkosky--Mehta co-lead ($-0.005$,
$[-0.058,+0.048]$ corrected) included. We report uncorrected intervals throughout and flag the
corrected verdicts here.

\paragraph{What predictability itself costs.} The claim that the predictable scale buys the
\emph{guarantee} rather than accuracy is measurable, and we measure it rather than assert it.
Normalizing by the \emph{post}-update scale $s_t$---which already contains $\norm{\gt}$, so it is
not $\mathcal F_{t-1}$-measurable and breaks the analysis---against the predictable $s_{t-1}$
\cref{alg:snomd} requires, on the same rows and the same grid with a paired block bootstrap
(\texttt{research\_predictability\_check.py}), moves the window-1 per-row score by
$+0.0001$ (CI $[-0.0017,+0.0018]$) at $\mathrm{lr}{=}2$ and $+0.0009$
($[+0.0007,+0.0012]$) at $\mathrm{lr}{=}1$. Enforcing measurability is therefore close to free
in accuracy here, which is what our framing predicts: predictability is what the
high-probability argument needs, not what the score rewards. This is one window and one method,
so it supports the framing rather than isolating the claim in general.
...
the ten-window ranking ($-0.005$, n.s.) and beats the remaining bounded baselines
($+0.075$ to $+0.103$ per-row, all clearing zero under correction), though its margin over the
deployed EMA default ($+0.046$) does \emph{not} survive that correction---while
...
removed by the strongly-adaptive wrapper of \cref{app:reductions}.

\subsection{The regime count grows linearly, which closes the switching route}
\label{app:regimecount}

\cref{rmk:regime} leaves the $\sqrt T$ drift factor to a switching bound in the regime count.
Such a bound would give $\tilde O(N^{1-1/p}T^{1/p})$: apply the per-regime $T_i^{1/p}$ guarantee
on each of $N$ regimes and sum, H\"older giving $\sum_i T_i^{1/p}\le N^{1-1/p}T^{1/p}$ with
equality at equal lengths. Both limits behave---$N{=}1$ returns $T^{1/p}$, $N{=}T$ returns the
vacuous $T$---so it improves on $T^{1/p+1/2}$ exactly when $N$ is \emph{sublinear} in $T$.

That is a measurable precondition, and it fails. Using this paper's own definition of a
regime---a horizon over which the tracker's upward variation is $O(1)$---we partition the
$200{,}000$-round scale process greedily, closing a segment when its accumulated upward
variation would exceed a budget $c$, and fit the growth of the segment count
(\texttt{research\_d1b\_regime\_count.py}). A single step larger than $c$ forms its own segment,
which is exactly the ``piecewise-slow variation plus arbitrary jumps'' model of
\cref{app:bracket}.

Across the deployed winsorized EMA and the two-timescale envelope, and $c$ swept over
$\{0.5,1,2,5,10\}$ median scales, $N$ grows with an exponent of \emph{at least} $1$ in $T$
(fitted $1.16$--$1.24$; $N\le T$ by construction, so a fitted value above $1$ is a
finite-horizon artifact and the claim we make is the lower bound). We apply
\cref{tab:beta}'s reliability rule---an exponent needs ${\gtrsim}50$ segments to mean
anything---which discards $7$ of the $15$ combinations, including every block-median one, where
the coarse $B{=}10^4$ update leaves one to three segments over the whole record. Note the
direction: that rule removes the only combinations whose exponent looked sublinear, so it makes
the conclusion harder to reach, not easier. Over the eight that survive it, the exponent never
falls below $1.15$; an exponent above $1$ cannot persist ($N\le T$), so the honest reading is
\emph{at least linear}. \cref{tab:beta}'s $\beta\approx1.0$--$1.25$ for $W_s$ agrees, which we
report as an internal consistency check and not as corroboration: $W_s$ counts upward moves and
$N$ counts the boundaries those same moves induce, so the two are close to the same measurement
twice.

At $N=\Theta(T)$ the switching rate is $N^{1-1/p}T^{1/p}=\Theta(T)$ for every $p$. The switching
bound is therefore not an open lemma on this data but an unavailable one, and the $\sqrt T$ that
makes \cref{thm:regret} per-regime is a property of the measured drift rather than of our proof.
What this does \emph{not} rule out is a bound in some other nonstationarity functional, or a
different stream on which regimes are rarer.

\paragraph{The same count empties \cref{prop:tracker}'s discharge.} \cref{prop:tracker} buys the
lower bracket at one added term $O(D\,N\,W\sup_t\sigma_t)$, ``dominated whenever $NW\ll
T^{1/p}$''. Both factors are now measured. $N$ is at least linear, as above. And $W$ is not free:
the blocking step needs ${\asymp}W/\ell$ near-independent blocks of length $\ell$ at least the
mixing time, and the scale process's rank autocorrelation is still $0.069$ at lag $2000$, so
$\ell\gtrsim2000$ and $W\gtrsim2\!\times\!10^4$. Asymptotically $NW/T^{1/p}$ grows like
$T^{1-1/p}$ even taking $N=\Theta(T)$ exactly rather than the fitted exponent, so it diverges at
every $p\in(1,2]$. At our own horizon, at $p{=}2$---the only value \cref{ass:moment} admits given
the measured normalized index---the ratio is $12$--$46$ using the tracker's own $W{=}64$, and
$3.8\!\times\!10^3$--$1.4\!\times\!10^4$ at the $W$ the proof requires. The two settings at which
the term \emph{is} dominated ($0.89$ and $0.46$) both need $p{=}1.3$, a heavier tail than we
measure, \emph{and} $W{=}64$, a shorter window than the blocking argument permits.

So \cref{prop:tracker} is not wrong; it is empty here. Its guarantee is real for a process with
macroscopic regimes and this process does not have them, which is why \cref{sec:tracker}'s
empirical discharge---$100\%$ lower-bracket coverage, measured---is what the paper actually
leans on. Coarsening the tracker does not escape either: at \cref{tab:beta}'s $B{=}10^4$ the
bracket holds at $0.93$, and charging the remaining $7\%$ of rounds trivially at
$O(D\sup_t\sigma_t)$ is $\Theta(T)$---the same wall reached by a different route.
...
\paragraph{One budget we did \emph{not} match: Cutkosky--Mehta.} The matched-budget rework
above equalized SN-OMD against the fixed-$\tau$ clip at $70$ configurations each. It did not
touch CM, which is tuned over its whole space $(\mathrm{lr},\tau,\beta)$---$9\times6\times4=216$
configurations on window~1, roughly three times the budget either capped method receives. We
leave it that way and disclose it rather than trimming CM's grid, for two reasons. Tuning a
method over its own free parameters is the principle the rest of the table follows, and CM
genuinely has three. And the asymmetry runs \emph{against} this paper: a larger window-1 budget
is more opportunity to overfit the tuning window, so if it biases the comparison it flatters CM,
which is the row we report ourselves as merely tying ($-0.005$, n.s.; \cref{app:tracker}). A
reader who wants the strictly matched comparison should read our tie as conservative. What we
cannot say from these runs is how CM would place at $70$ configurations, and we do not claim it.
