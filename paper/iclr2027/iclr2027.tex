...
or realization; a stability, not convergence, guarantee). The predictable scale further buys a
high-probability dynamic-regret bound at the per-regime rate $T^{1/p}$ ($p\in(1,2]$) under a
finite $p$-th moment and a predictable scale-tracking condition. Empirically, a whole family of \emph{bounded scale-free} updates stays
bounded and accurate where OGD and scale-dependent clipping diverge; across ten disjoint windows
with frozen hyperparameters every finite-cap member stays bounded ($0/10$ divergences) while the
uncapped endpoint and OGD do not, and the measurement and the stability partition both replicate
on a second, independent market. Two limits we state rather than bury: the regret rate is
\emph{per regime}, the measured scale drift entering it as a genuine $\sqrt T$ factor, and
per-row SN-OMD's block-median scale \emph{ties} the best bounded baseline (momentum-normalized
SGD) rather than beating it---the contribution is the measurement and the stability, not an
accuracy win.
\end{abstract}
...
gradient-norm tail is genuinely heavy ($\hat\alpha\approx2.4$)\footnote{Throughout, $\hat\alpha$
is a Hill estimate of an \emph{observed marginal}'s tail index; the exponent $p$ of
\cref{ass:moment} is the \emph{conditional} moment exponent the theory assumes finite. We report
$\hat\alpha$ as evidence about $p$, not as an estimate of it.} but \emph{conditionally
...
\emph{predictable} part of a nonstationary scale. The
...
\begin{itemize}
  ...
  artifact). The heavy marginal is volatility clustering---a bounded drift cannot produce it
  ...
  \item \textbf{A protocol-careful study (\cref{sec:experiments}).}
  ...
  control. A per-regime dynamic-regret bound (\cref{thm:regretbody}) is what the
  \emph{predictable} scale buys, honest about a $\sqrt T$ drift factor; a tested implementation
is provided as anonymized supplementary material.
\end{itemize}
...
constant-threshold clip, and AdaGrad-Norm (\cref{tab:jane})---so it separates the bounded
...
Tail measurements are learner-independent---gradients at the fixed $w^\star$ (pooled index from
${\sim}200{,}000$ gradients, daily-drift index from the full record), so the tail is a property of
the data, not of any learner; we do not track it along the trajectory $\{w_t\}$, whose norm
...
battery of look-ahead controls (\cref{tab:leakage} and \cref{app:tables}, all on the reported
window): a shuffled-label null, static and per-day look-ahead ridge oracles, and a
frozen-weights forward test. All are consistent with \emph{no} label leakage, and all three
oracles fall short of the online score. The reported $R^2$ is thus a \emph{tracking} score on a
fast-drifting stream, not a transferable static fit---itself evidence for the nonstationarity we
study, and why it is not comparable to the competition's protocol.
...
by \emph{causal-estimation cost} rather than a resolvable residual tail: the \emph{same} estimator
run non-causally recovers only $\sim\!+0.3$, while on a GARCH surrogate with Gaussian
innovations---\emph{zero} residual tail by construction---the same causal tracker still reads
$\hat\alpha\approx3.9$--$4.5$ against a true-$\sigma_t$ oracle of $\approx9$. The real $3.73$
sits just \emph{below} that zero-residual band, by about the surrogate's own sensitivity to its
persistence, so we cannot separate a genuine residual conditional tail from a scale merely less
predictable than GARCH(1,1); either way the residual is dominated by estimation cost and
$\hat\alpha\approx3.7$ leaves $p>2$. What makes it removable is \emph{serial dependence}: the gradient magnitude is strongly clustered---rank (Spearman)
...
\emph{predictable} part of the scale. (Per-day $\hat\alpha$ dips below $2$ on a minority of days,
but single-day Hill estimates are noisy and we do not rely on them; the per-regime budget
heuristic of \cref{sec:lower} rests on exactly those days and we keep it qualitative for that
reason.)

\paragraph{The mechanism replicates on a second, independent market.} On Binance BTC/USDT hourly
bars ($76{,}888$ bars, 2017--2025) the same analysis replicates more sharply: the pooled gradient
tail is \emph{heavier} than Jane's ($\hat\alpha\approx1.81$, i.e.\ infinite variance) and again
heavier than either factor (residual $2.68$, feature $3.94$---the same interaction effect); the
causal tracker lightens it to $\approx2.26$; and the decisive control holds---a
marginal-preserving order-\emph{shuffle} abolishes the lightening (real
$\Delta\hat\alpha={+}0.45$ vs.\ shuffled ${-}0.01\!\pm\!.02$ over five seeds). So does the
stability partition (OGD diverges $10/10$, the uncapped endpoint $3/10$, every bounded scale-free
method $0/10$). Crypto's residual \emph{does} carry a genuine heavy innovation tail that
scale-tracking does not remove, unlike Jane's; that honest scope difference and the full
replication are in \cref{app:secondmarket}.
...
($0.162\to0.080$: aggregating enlarges the untruncated step a finite cap would contain).
...
$[-0.014,+0.023]$)---and on this one protocol only. These are distinct claims: predictability is
what \cref{thm:regretbody} requires, and is in principle orthogonal to how accurate any
particular predictable tracker is, so the block median's co-lead is an empirical property of
\emph{that} tracker (\cref{sec:tracker}), not a consequence of predictability. \emph{(ii) The cap:} a
...
SN-OMD's per-row error bar runs about twice normalized-GD's ($\pm.079$ vs.\ $\pm.037$;
\cref{tab:jane}, and no wider per-step), which the replication traces to the winsorized-EMA
tracker rather than the cap---the block median roughly halves it
...
rate ($\mathrm{lr}{=}0.2$ vs.\ $0.5$ per-row), so it underfits ($0.09\!\pm\!.02$) instead of
diverging; the finite cap is stable at every rate on \emph{both} protocols. So ``OGD
...
the same guarantee-versus-accuracy gap stated there.
...
\citet{cutkoskymehta2021} \emph{co-lead} in a statistical tie, both clear of the rest; per-step no
member leads decisively (\cref{sec:experiments,app:tracker}). Rates are tuned on window~1 over a
grid with all \emph{accuracy-relevant} optima interior (the boundary flags, and why they are not
truncated optima, in \cref{app:grid}); CM at $\beta{=}0$ \emph{is} normalized-GD (bit-identical
...
\paragraph{Further experiments (deferred to \cref{app:further}).} Two supporting studies develop
(i)~the \emph{cap frontier}---SN-OMD interpolates normalized-GD ($M\to0$) and uncapped
scale-adaptive OGD ($M\to\infty$), only the uncapped endpoint diverges, and an $M$-sweep gives a
\emph{sharp} interior accuracy optimum under an imposed heavy tail but a \emph{broad} one on
Jane's milder residual; and (ii)~the empirical discharge of \cref{ass:track}: a two-timescale
peak-hold envelope holds the stability-critical lower bracket at $100\%$ of rounds with
$W_s\approx6.6\,V_\sigma^+$ growing \emph{linearly} in $T$, which is why
...
head-to-head across the ten frozen windows (tuned over its momentum coefficient), their method
stays bounded ($0/10$) and \emph{co-leads} per-row, statistically tied with the block-median
SN-OMD (\cref{tab:replication,app:tracker}), so it belongs squarely in the bounded scale-free
family. That co-lead is
...
\paragraph{Adaptive optimizers.} Coordinate-wise adaptive methods (AdaGrad, RMSProp, Adam) also
rescale by a running second moment. We ran RMSProp and Adam head-to-head rather than reasoning
about them, which corrected an argument we had made from the armchair (\cref{app:adaptive}):
sharing the round \emph{bounds} the step rather than inflating it, since
$v_t=\rho v_{t-1}+(1-\rho)\gt^2$ forces $|g_{t,i}|/\sqrt{v_{t,i}}\le(1-\rho)^{-1/2}$ however
extreme the gradient. RMSProp and Adam are therefore \emph{bounded-step} methods that do not
diverge---their peak rolling loss grows like $\eta^2$ but stays finite where OGD's reaches
$10^{300}$---and they \emph{tie} SN-OMD once the step schedule is matched, the fourfold gap a
naive comparison shows being the schedule, not the preconditioner. They belong in the bounded
family; what they lack, like \emph{AdaGrad-Norm} (monotone accumulator, step bounded by $\eta$,
$0.20\!\pm\!.07$ per-row; \cref{tab:replication}), is a \emph{predictable} scale and the
dynamic-regret guarantee (\cref{thm:regretbody}).
...
comparable to competition scores (different protocol). Because every method is at \emph{its own}
tuned rate, one method can appear at different rates in different experiments: normalized-GD's
per-row optimum is $\mathrm{lr}{=}3$ on the baseline grid used here ($0.220\!\pm\!.037$) and
$\mathrm{lr}{=}2$ on the cap-frontier grid where it is the $M\!\to\!0$ endpoint
($0.197\!\pm\!.024$; \cref{sec:msweep}). Both are that experiment's own optimum, not a
duplicated or stale row.}
\label{tab:jane}
\centering
\begin{small}
\setlength{\tabcolsep}{3.5pt}
...
$\alpha/14$ ($t_9$ critical value $3.91$ instead of $2.26$) changes exactly one verdict: the
per-step gap over the uncapped scale-adaptive endpoint, $+0.106$ with uncorrected CI
$[+0.005,+0.208]$, becomes inconclusive ($[-0.069,+0.282]$). Every gap the paper's claims rest on
...
report uncorrected intervals and flag the one borderline result as suggestive.
...
optimum is strictly \emph{interior}; the remaining boundary flags are not truncated optima
(notes).}
\label{tab:grid}
\centering
\begin{small}
...
$M{=}2$ (range $[2,20]$), so on that protocol a smaller cap---toward the $M\!\to\!0$
normalized-GD endpoint---is untested. We do not tune $M$ (fixed at $5$ a priori), and the
ten-window \emph{per-row} sweep puts $M{=}5$ on a broad replicated plateau, so this does not
enter any reported number; it does mean the ``interior cap optimum'' claim is a per-row one.}
\end{table}
...
$\beta{=}(0.9,0.999)$, $\varepsilon{=}10^{-8}$, same window, protocols, grids and circular block
bootstrap as \cref{tab:jane}) and the argument was wrong. It is wrong for a reason worth stating,
...
$\eta/\sqrt t$ score is the $0.285$ reported there rather than $0.247$---the $0.247$ is this
sweep's grid, not a competing estimate of the same quantity. AdaGrad-Norm's accumulator already
...
column is a standing invitation: SN-OMD itself scores $0.52$ there against $0.28$ under the
