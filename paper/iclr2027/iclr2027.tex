...
\begin{theorem}[Dynamic regret, simplified]
\label{thm:regretbody}
Assume \cref{ass:moment}, and that $s_t$ is predictable and brackets the local scale
(\cref{ass:track}). Set the cap $M=\Theta(T^{1/p})$, or use a doubling schedule, which needs no
knowledge of $p$. Fix a comparator sequence $u_{1:T}$ of path length $P_T$, and a horizon over
which the tracker's upward variation is $O(1)$---a \emph{regime}, formalized in
\cref{app:bracket}. Then with probability $\ge1-\delta$,
\[
  R_T(u_{1:T})\;=\;\tilde O\!\bigl((\textstyle\sup_t\sigma_t)\,(D+\sqrt{D\,P_T})\;T^{1/p}
  \log(1/\delta)\bigr).
\]
\end{theorem}
\cref{thm:regret} in \cref{app:regret} gives the unsimplified constants, carrying $\sqrt{W_s}$
and the peak-to-mean scale ratio explicitly, and \cref{app:proofs} gives the proof. Two of the
bound's costs belong here rather than there.

\emph{(i) It holds per regime, not globally.} The bound is carried by the tracker's upward
variation $W_s=s_1+\sum_t(s_t-s_{t-1})_+$, which we \emph{measure} to grow linearly in $T$ on
real data (\cref{sec:tracker}). Over a horizon with a growing number of regimes the exponent
therefore becomes $T^{1/p+1/2}$, which is \emph{vacuous} at $p{=}2$.

\emph{(ii) It rests on an assumption, and on a restricted comparator.} \cref{ass:track} is
one-sided in what matters and is dischargeable (\cref{prop:tracker}), but it remains an
assumption; and the guarantee holds per \emph{fixed predictable} comparator, not against the
per-round minimizer (\cref{rmk:scope}).
...
(\cref{fig:mechanism}). Frozen across the ten windows the clippers diverge $9/10$ per-step, the
same count as OGD (\cref{app:family}).
...
cap is for stability alone---and the per-regime restriction on \cref{thm:regretbody} is not
...
Scale-invariance is a separate matter and governs whether a \emph{tuned rate transfers}: if
$\Phi$ is positively homogeneous of degree one---as plain OGD is, and as the clippers are, since
$\tau_t=c\,s_t$ scales with the stream---then running rate $\eta$ on a $\lambda$-scaled stream
reproduces rate $\lambda\eta$ on the original, so the stable-rate set contracts as $1/\lambda$;
if $\Phi$ is degree zero the stable-rate set does not move at all.
