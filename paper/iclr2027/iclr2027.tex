...
does not manufacture SN-OMD's accuracy: aggregating the $\sim\!9$ correlated symbols per
timestep leaves SN-OMD essentially unchanged ($0.285\!\to\!0.307$), so no contemporaneous
...
protocol only (intervals and window counts in \cref{app:tracker}). These are distinct claims: predictability is
...
member leads decisively (\cref{sec:experiments,app:tracker}). Each method is tuned over its own
free parameters: both SN-OMD rows over $(\mathrm{lr},M)$, the same two-parameter budget
fixed-$\tau$ gets over $(\mathrm{lr},\tau)$, and the threshold-free methods over $\mathrm{lr}$
alone---their whole parameter space. Every selected optimum is interior except the per-step
SN-OMD row, which pins at both grid edges (\cref{app:grid}). CM at $\beta{=}0$ \emph{is}
normalized-GD, so their per-step cells are bit-identical rather than duplicated.}
...
scale-adaptive OGD ($M\to\infty$), and an $M$-sweep locates a \emph{sharp} interior accuracy
optimum under an imposed heavy tail but a \emph{broad} one on Jane's milder residual; and
(ii)~the empirical discharge of \cref{ass:track}, which measures the tracker's upward variation
$W_s$ growing \emph{linearly} in $T$---the reason \cref{thm:regretbody}'s rate is per-regime.
...
causal tracker still reads $\hat\alpha\approx3.9$--$4.5$ against a true-$\sigma_t$ oracle of
$\approx9$. The real $3.73$ sits just \emph{below} that zero-residual band, by about the
...
\texttt{research\_null\_normalization.py} in the anonymized supplement. Finally, per-day
