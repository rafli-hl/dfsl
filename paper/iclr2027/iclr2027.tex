...
such a bound. OGD is fragile here: stable only to $\mathrm{lr}\approx10^{-2}$, best
$R^2\approx0.02$--$0.14$, diverging above that when the scale spikes (\cref{fig:mechanism}); the
clippers we sweep on a shorter window only, so we report accuracy but locate no ceiling.
...
(\cref{tab:replication}). $^\dagger$Swept on date$[0,30)$, not this stream, so no ceiling is
located for these two. $R^2$ is not comparable to competition scores.}
\label{tab:jane}
\centering
\begin{small}
\setlength{\tabcolsep}{3.5pt}
...
AdaptiveClip / RobustOMD$^\dagger$   & $\sim0.01$ & --- & --- \\
\midrule
...
diverge, only its upper bound to be loose and its tuning not to transfer. On our own stream a
small enough constant rate keeps every scale-dependent method bounded.
