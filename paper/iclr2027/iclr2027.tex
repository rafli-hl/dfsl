...
\cref{tab:jane}, and no wider per-step). The replication does not trace this to the tracker:
once the cap is tuned alongside the rate, the deployed winsorized-EMA is the \emph{tighter}
of the two SN-OMD rows across windows (\cref{tab:replication,sec:tracker}).
...
their window-1-optimal rates having overfit that window (\cref{app:grid}). With its cap tuned alongside the rate, SN-OMD's
\emph{deployed} winsorized-EMA tracker is the \emph{less} variable of the two SN-OMD rows
per-row ($0.24\!\pm\!0.05$ against the block median's $0.28\!\pm\!0.09$), and neither is negative
on any window; both keep $0/10$ divergences. The block median buys ${\approx}{+}0.04$ of mean at
the cost of spread, so the tracker choice trades accuracy against variability rather than
dominating. The more accurate tracker remains the one \emph{without} a proven $W_s$ bound
(\cref{sec:tracker}), the same guarantee-versus-accuracy gap stated there.
...
member leads decisively (\cref{sec:experiments,app:tracker}). Both SN-OMD rows are tuned over $(\mathrm{lr},M)$, the same two-parameter
budget fixed-$\tau$ gets over $(\mathrm{lr},\tau)$; methods with no threshold parameter are tuned
over $\mathrm{lr}$ alone, their whole parameter space. All \emph{accuracy-relevant} optima are
interior (\cref{app:grid}); CM at $\beta{=}0$ \emph{is} normalized-GD (bit-identical
...
most accurate is exactly the one we cannot yet bound.
